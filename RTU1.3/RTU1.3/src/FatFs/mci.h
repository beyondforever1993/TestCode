...
#else
#define MCLKDIV_SLOW		60-1 /* 59 = 400,000Hz -> @48Mhz/(2*60) */
#define MCLKDIV_NORMAL		5-1  /* 2 = 8Mhz -> @48Mhz/(2*3) */
#endif
...
int32_t SD_Set_BusWidth( uint32_t width );
int32_t MCI_Cmd_SendAcmdOpCond(uint8_t hcsVal);
int32_t MCI_CardInit( en_Mci_CardType *cType );
...
int32_t MCI_GetCardStatus(uint32_t* cardStatus);
uint32_t MCI_GetBlockEndFlag(void);
int32_t MCI_SetBlockLen( uint32_t blockLength );
int32_t MCI_Cmd_SendAcmdBusWidth( uint32_t buswidth );
int32_t MCI_Cmd_StopTransmission( void );

int32_t MCI_WriteBlock(uint8_t* memblock, uint32_t blockNum );
int32_t MCI_ReadBlock(uint8_t* destBlock, uint32_t blockNum );
