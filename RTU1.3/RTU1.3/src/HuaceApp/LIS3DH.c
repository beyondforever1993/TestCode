...
#define WTH_set 20
#define calibration_set 200
u8_t sensor_Dog = 0;
u8_t calibration_ok = 0;
u8_t calibration_num = 0;
AxesRaw_add Axes_calibration = {0,0,0};
AxesRaw_add Axes_last = {0,0,0};
AxesRaw_add Zero_offset = {0,0,0};
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
...
void I2C_init(en_I2C_unitId I2C_num, u32_t i2cClockFreq)
{
    I2C_Cmd(I2C_num, DISABLE);
    PINSEL_ConfigPin (0, 27, 1);
    PINSEL_ConfigPin (0, 28, 1);

    I2C_Init(I2C_num, i2cClockFreq);
    /* Enable I2C1 operation */
    I2C_Cmd(I2C_num, ENABLE);
    return;
}

status_t readByte( u8_t regAddr,  u8_t* data)
{
    I2C_M_SETUP_Type i2cData;
	i2cData.sl_addr7bit = LIS3DH_MEMS_I2C_ADDRESS;
	i2cData.tx_data = &regAddr;
	i2cData.tx_length = 1;
	i2cData.rx_data = data;
	i2cData.rx_length = 1;
	i2cData.retransmissions_max = 3;
	return (status_t)I2C_MasterTransferData(I2C_0, &i2cData, I2C_TRANSFER_POLLING);
}
status_t writeByte( u8_t regAddr, u8_t* data)
{
    I2C_M_SETUP_Type i2cData;
	u8_t data1[2];
	data1[0] = regAddr;
...
{
    return readByte(Reg, Data);
}

status_t LIS3DH_WriteReg(u8_t WriteAddr, u8_t Data)
{
    return writeByte(WriteAddr, &Data);
}

status_t LIS3DH_init(void)
{
    I2C_init(I2C_0,I2C0_Freq);
    GPIO_SetDir(BRD_SENSOR_INT_PORT,  BRD_SENSOR_INT_MASK, GPIO_DIRECTION_INPUT);

    if( !LIS3DH_SetODR(LIS3DH_ODR_400Hz) )
        return MEMS_ERROR;
    if( !LIS3DH_SetAxis(LIS3DH_X_ENABLE | LIS3DH_Y_ENABLE | LIS3DH_Z_ENABLE) )
        return MEMS_ERROR;
    if( !LIS3DH_SetMode(LIS3DH_NORMAL) )
        return MEMS_ERROR;
    if( !LIS3DH_SetFilterDataSel(MEMS_ENABLE) )
        return MEMS_ERROR;
    if( !LIS3DH_SetHPFMode(LIS3DH_HPM_REF_SIGNAL) )
        return MEMS_ERROR;
    if( !LIS3DH_SetHPFCutOFF(LIS3DH_HPFCF_2) )
        return MEMS_ERROR;
    if( !LIS3DH_HPFAOI1Enable(MEMS_ENABLE))
        return MEMS_ERROR;
    if( !LIS3DH_SetBDU(MEMS_ENABLE))
        return MEMS_ERROR;
    if( !LIS3DH_FIFOModeEnable(LIS3DH_FIFO_BYPASS_MODE) )
        return MEMS_ERROR;
    if( !LIS3DH_SetInt1Pin(LIS3DH_WTM_ON_INT1_ENABLE) )
        return MEMS_ERROR;
    if( !LIS3DH_SetFullScale(LIS3DH_FULLSCALE_2) )
        return MEMS_ERROR;
    if( !LIS3DH_SetWaterMark(WTH_set) )
        return MEMS_ERROR;
    if( !LIS3DH_FIFOModeEnable(LIS3DH_FIFO_MODE) )
        return MEMS_ERROR;

    return MEMS_SUCCESS;
}

status_t data_effect(AxesRaw_t *Acc)
{
    u32_t square_sum = 0;
    i16_t *value;
    value = (i16_t*)Acc;
    square_sum += (i32_t)*value * (i32_t)*value;
    value = (i16_t*)Acc + 1;
    square_sum += (i32_t)*value * (i32_t)*value;
    value = (i16_t*)Acc + 2;
    square_sum += (i32_t)*value * (i32_t)*value;
    if(abs(square_sum - 268435456) < 26843546 * 2)
        return MEMS_SUCCESS;
    else
        return MEMS_ERROR;
}

status_t Acc_to_angle(AxesRaw_add *Acc,Axes_angle *angle)
{
    float square_sqrt = 0;
    i32_t *value;
    i32_t *angle_point;
    value = (i32_t *)Acc;
    angle_point = (i32_t *)angle;
    square_sqrt = sqrt((*(value) * *(value))  +  (*(value + 2) * *(value + 2)));
    *(angle_point + 1) = (i32_t )(atanf((float)*(value + 1) / (float)square_sqrt) * 206265);
    square_sqrt = sqrt((*(value + 1) * *(value + 1))  +  (*(value + 2) * *(value + 2)));
    *(angle_point) = (i32_t )(atanf((float)*value / (float)square_sqrt) * 206265);
    return MEMS_SUCCESS;
    //return MEMS_ERROR;
}
