...
#define PLL0_SETUP            1
#define PLL0CFG_Val           0x00000009 //xf 12*x+1 9:120M  7:96M
#define PLL1_SETUP            1
...
{
    uint32_t cclksel,pclksel,emcclksel,usbclksel;
  ...
  if ((LPC_SC->CCLKSEL &0x100) == 0) {            /* cclk = sysclk    */
    ...
    if ((LPC_SC->PLL0STAT & 0x100) == 0) {        /* PLL0 not enabled */
      ...
      if ((LPC_SC->CLKSRCSEL & 0x01) == 0) {    /* sysclk = irc_clk */
          cclksel = LPC_SC->CCLKSEL;//add by xxw 20140815 两个volatile类型变量是不能同时出现在一个等式里的
          pclksel = LPC_SC->PCLKSEL;
          emcclksel = LPC_SC->EMCCLKSEL;
          SystemCoreClock = (IRC_OSC * ((LPC_SC->PLL0STAT & 0x1F) + 1) / (cclksel & 0x1F));
          PeripheralClock = (IRC_OSC * ((LPC_SC->PLL0STAT & 0x1F) + 1) / (pclksel & 0x1F));
          EMCClock        = (IRC_OSC * ((LPC_SC->PLL0STAT & 0x1F) + 1) / ((emcclksel & 0x01)+1));
      }
      else {                                        /* sysclk = osc_clk */
        if ((LPC_SC->SCS & 0x40) == 0) {
          SystemCoreClock = 0;                      /* this should never happen! */
          ...
          cclksel = LPC_SC->CCLKSEL;
          pclksel = LPC_SC->PCLKSEL;
          emcclksel = LPC_SC->EMCCLKSEL;
          SystemCoreClock = (OSC_CLK * ((LPC_SC->PLL0STAT & 0x1F) + 1) / (cclksel & 0x1F));
          PeripheralClock = (OSC_CLK * ((LPC_SC->PLL0STAT & 0x1F) + 1) / (pclksel & 0x1F));
          EMCClock        = (OSC_CLK * ((LPC_SC->PLL0STAT & 0x1F) + 1) / ((emcclksel & 0x01)+1));
        }
      }
    }
  }
  ...
  if(LPC_SC->USBCLKSEL & (0x01<<8))//Use PLL0 as the input to the USB clock divider
  {
	  ...
	  case 4:
	  case 6:
          usbclksel = LPC_SC->USBCLKSEL;
		  if(LPC_SC->CLKSRCSEL & 0x01)	//pll_clk_in = main_osc
			  USBClock = (OSC_CLK * ((LPC_SC->PLL0STAT & 0x1F) + 1) / (usbclksel & 0x1F));
		  else //pll_clk_in = irc_clk
			  USBClock = (IRC_OSC * ((LPC_SC->PLL0STAT & 0x1F) + 1) / (usbclksel & 0x1F));
		  break;
	  default:
