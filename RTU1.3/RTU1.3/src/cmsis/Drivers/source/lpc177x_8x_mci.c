...
uint32_t dmaWrCh_TermianalCnt = 0, dmaWrCh_ErrorCnt = 0;
uint32_t dmaRdCh_TermianalCnt = 0, dmaRdCh_ErrorCnt = 0;


uint32_t MCI_SettingDma(uint8_t* memBuf, uint32_t ChannelNum, uint32_t DMAMode ,uint32_t numOfBlock);
...
uint32_t MCI_SettingDma(uint8_t* memBuf, uint32_t ChannelNum, uint32_t DMAMode,uint32_t numOfBlock )
{
  ...
  // Transfer size
  GPDMACfg.TransferSize = DMA_MCI_SIZE * numOfBlock;
  // Transfer width
...
{
  volatile uint32_t i;
  uint32_t bus_width = BUS_WIDTH_1BIT;
  ...
  {
    LPC_MCI->CLOCK &=  ~(1 << 11);    /* 1 bit bus */
    bus_width = 0x03B70000;
  }
  else if ( width == SD_4_BIT )
  {
    LPC_MCI->CLOCK |=  (1 << 11);/* 4 bit bus */
    bus_width = 0X03B70100;
  }
  ...
  else if( MCI_CardType == MCI_MMC_CARD)
  {
    //针对MMC修改
    volatile uint32_t i;
    uint32_t retryCount;
...
/************************************************************************//**
* @brief        Set the number of blocks to be transfered befor block read OR
*               block write 
*
* @param[in]    block_cnt the value for the nu of block will be handled
*
* @return         MCI_FUNC_OK if all success
*
* @note         CMD23 command should be sent immediately before multiple block
*               read OR block write 
****************************************************************************/
int32_t MCI_SetBlockCount( uint32_t block_cnt)
{
  volatile uint32_t i;
  uint32_t retryCount;
  uint32_t respStatus;
  ...
  retryCount = 0x20;
  cmdIf.CmdIndex = CMD23_SET_BLOCK_COUNT;
  cmdIf.Argument = block_cnt;
  cmdIf.ExpectResp = EXPECT_SHORT_RESP;
  cmdIf.AllowTimeout = ALLOW_CMD_TIMER;
  cmdIf.CmdResp =  (uint32_t *)&respValue[0];
  while ( retryCount > 0 )
...
int32_t MCI_Cmd_WriteBlock(uint32_t blockNum, uint32_t numOfBlock)
{
  ...
  while ( retryCount > 0 )
  {
    ...
    if (numOfBlock > 1)
    {
      if( MCI_SetBlockCount( numOfBlock & 0xFFFF) != MCI_FUNC_OK )
      {
        return ( MCI_FUNC_FAILED );
      }
    }
...
int32_t MCI_WriteBlock( uint8_t* memblock, uint32_t blockNum, uint32_t numOfBlock)
{
  ...
  {
#if MCI_DMA_ENABLED
    MCI_SettingDma((uint8_t*) dataSrcBlock, MCI_DMA_WRITE_CHANNEL, GPDMA_TRANSFERTYPE_M2P_DEST_CTRL,numOfBlock);
...
int32_t MCI_ReadBlock( uint8_t* destBlock, uint32_t blockNum, uint32_t numOfBlock)
{
  ...
  {        
#if MCI_DMA_ENABLED
    MCI_SettingDma((uint8_t*) dataDestBlock, MCI_DMA_READ_CHANNEL, GPDMA_TRANSFERTYPE_P2M_SRC_CTRL,numOfBlock);
