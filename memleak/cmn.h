...
#define TRACE
#define DLL
//#define WIN32  mark:It has defined in vs
#ifndef CODELIB_H
#define CODELIB_H
#include <stdio.h>
#include <stdlib.h>
...
#define VER_CODELIB  "2.4.2"             /* library version */

#define COPYRIGHT_CODELIB \
    "Copyright (C) 2014-2015 by HC\nAll rights reserved."
...
#define CODE_L1I    47
#define CODE_L1Q    48
#define MAXCODE     49                  /* max number of obs code */
...
#define ARMODE_ROUND 8                  /* AR mode:round*/
...
#define STRFMT_UB370 19                 /* tya,ub370 */
#define STRFMT_TRIMBLE 20
#ifndef EXTLEX
#define MAXRCVFMT    11                 /* max number of receiver format */
#else
#define MAXRCVFMT    12
#endif
#define RT17BUFLEN (255+4+2)            /*RT17 buffer length*/
...
#define MAX_RNX_DAT_BUF_LEN  ( 100 * 1024 )
#define MAX_HRC_DAT_BUF_LEN  ( 5 * 1024 )

enum {
	RNX_CONV_DAT_TYPE__NONE,
	RNX_CONV_DAT_TYPE__OBS,
    RNX_CONV_DAT_TYPE__SYS,
	RNX_CONV_DAT_TYPE__GLO,
	RNX_CONV_DAT_TYPE__SBS,
	RNX_CONV_DAT_TYPE__QZS,
	RNX_CONV_DAT_TYPE__GAL,
	RNX_CONV_DAT_TYPE__CMP,
	RNX_CONV_DAT_TYPE__METE
};


typedef struct _rnx_dat_buf {
	char *cur;
	int  dat_type;
	int  len;
	char buf[ MAX_RNX_DAT_BUF_LEN ];
} rnx_dat_buf_t;

typedef struct _hrc_dat_buf {
	unsigned char *cur;
	int  len;
	unsigned char buf[ MAX_HRC_DAT_BUF_LEN ];
} hrc_dat_buf_t;
...
typedef struct          /* observation data record */
{
    ...
    unsigned char sat, rcv; /* satellite/receiver number (1:rover;2:base)*/
...
typedef struct          /* GPS/QZS/GAL broadcast ephemeris type */
{
    ...
    double toes, tocs;        /* Toe (s) in week */
...
typedef struct          /* GLONASS broadcast ephemeris type */
{
    ...
    int leaps;          /* leap seconds:tya add*/
} geph_t;
...
typedef struct {        /* rinex control struct type */
	char   senserMod[32];
	char   senserTyp[32];//senser type
	float  senserAcur;   //senser accuracy
} chc_mete_senserInfo;

typedef struct {        /* rinex control struct type */
	gtime_t time;       /* epoc time */
	float PR;//ÆøÑ¹£¨mbar£©
	float TD;//ÎÂ¶È(¡ãC)
	float HR;//Ïà¶ÔÊª¶È(%)
	float ZW;//Ìì¶¥ÊªÑÓ³Ù(mm)
	float ZD;//Ìì¶¥ÑÓ³ÙµÄ¸É·ÖÁ¿(mm)
	float ZT;//×ÜÌì¶¥ÑÓ³Ù(mm)
	float WD;//·çÏò·½Î»½Ç£¨deg£©
	float WS;//·çËÙ£¨m/s£©
	float RI;//ÓêÔöÁ¿£¨1/10mm£©
	float HI;//±ù±¢Ö¸±ê£¨Hail indicator£©£º1=´ÓÉÏÒ»´Î¹Û²âÖÐÌ½²â³öÓÐ±ù±¢
} chc_mete_epot;
typedef struct {        /* rinex control struct type */
	gtime_t time;       /* message time */
	double ver;         /* rinex version */
	char   programNa[21];//program name to create the rinex
	char   companyNa[21];//company name to create the rinex
	//char   createDat[21];//time to create the rinex
	chc_mete_senserInfo senserInfo[3];
    int   n;        /* number of mete data/allocated */
	double H; //observation pos
	double PR;//ÆøÑ¹£¨mbar£©
	double TD;//ÎÂ¶È(¡ãC)
	double HR;//Ïà¶ÔÊª¶È(%)

	//chc_mete_epot  *data;         /* mete data body*/
} mete_t;
...
typedef struct          /* RINEX options type */
{
    ...
    char metecompanyNa[32];  /* metelogical company name */
    char metesenserInfo[3][32]; /* metelogical sensor info */
} rnxopt_t;
...
typedef struct          /* satellite status type */
{
    ...
    unsigned char slip[NFREQ]; /* cycle-slip flag(1:occur slip) */
...
typedef struct
{
    ...
    int nr;
    int ns;
    double y[MAXSAT * 10 * 2];
    double e[MAXSAT * 3 * 2];
    double azel[MAXSAT * 2 * 2];
} tripsolveinfo_t;
typedef struct
{
    char type;  /*tya add:rtk obs type,0-L1;1-WL12,2-WL23,3-WL123*/
    int i;
    int j;
    int k;
...
typedef struct          /* RTK control/result type */
{
    ...
    char mode;           /*tya add:rtk obs type,0-L1;1-WL12_1,2-WL12,3-WL23_12,4-W23,5-WL123*/
    lc_t lc;
    double *ixa;
    int ina;
    tripsolveinfo_t trfo;
    psol_t psol;
} rtk_t;
...
typedef struct          /* receiver raw data control type */
{
    ...
    nav_t nav;          /* satellite ephemerides */
    sta_t sta;          /* station parameters */
    mete_t mete;        /* metelogical parameters */
    ...
    unsigned char pbuff[RT17BUFLEN]; /* RT17: Packet buffer */
} raw_t;
...
extern int outrnxobsb( FILE *fp, const rnxopt_t *opt, const obsd_t *obs, int n,
                       int epflag );
extern int outrnxobsb_buf( rnx_dat_buf_t *dat_buf, const rnxopt_t *opt, const obsd_t *obs, int n,
                       int epflag );                     
extern int outrnxnavh ( FILE *fp, const rnxopt_t *opt, const nav_t *nav );
extern int outrnxcnavh( FILE *fp, const rnxopt_t *opt, const nav_t *nav ); //tya
...
extern int outrnxhnavh_buf( rnx_dat_buf_t *dat_buf, const rnxopt_t *opt, const nav_t *nav );
extern int outrnxlnavh( FILE *fp, const rnxopt_t *opt, const nav_t *nav );
extern int outrnxlnavh_buf( rnx_dat_buf_t *dat_buf, const rnxopt_t *opt, const nav_t *nav );
...
extern int outrnxmeteb_buf( rnx_dat_buf_t *dat_buf, const rnxopt_t *opt, const mete_t *mate_t, int n,
					   int flag );
...
extern int outrnxnavb_buf ( rnx_dat_buf_t *dat_buf, const rnxopt_t *opt, const eph_t *eph );
extern int outrnxgnavb( FILE *fp, const rnxopt_t *opt, const geph_t *geph );
extern int outrnxgnavb_buf( rnx_dat_buf_t *dat_buf, const rnxopt_t *opt, const geph_t *geph );
extern int outrnxhnavb( FILE *fp, const rnxopt_t *opt, const seph_t *seph );
extern int outrnxhnavb_buf( rnx_dat_buf_t *dat_buf, const rnxopt_t *opt, const seph_t *seph );
...
extern int input_oem4_zy( raw_t *raw, unsigned char *data, int len, int *read_len );
...
extern int input_ub370  ( raw_t *raw, unsigned char data ); /*tya 141128*/
extern int input_ub370_zy( raw_t *raw, unsigned char *data, int len, int *read_len );
extern void set_obstype( int format, rnxopt_t *opt ); /*tya*/
extern void convobs( FILE **ofp, rnxopt_t *opt, const raw_t *raw, int *n ); /*tya*/
extern void convobs_buf( rnx_dat_buf_t *dat_buf, rnxopt_t *opt, const raw_t *raw, int *n  );
extern void closefile( FILE **ofp, const rnxopt_t *opt, nav_t *nav ); /*tya*/
extern void closelinkfile( FILE **ofp ); /*zy*/
extern void convnav( FILE **ofp, rnxopt_t *opt, raw_t *raw, int *n ); /*tya*/
...
extern void setapppos( raw_t *raw, rnxopt_t *opt ); /*tya*/
extern int showstat( int sess, gtime_t ts, gtime_t te, int *n ); /*tya*/
extern void outheader( FILE **ofp, const rnxopt_t *opt, nav_t *nav ); /*tya*/
