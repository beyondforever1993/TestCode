...
#define ID_BDSEPHEMERIS 1583
#define ID_BDSEPHEMERIS_OEM6 1696
...
{
    int i;
    for ( i = 0; i < 15; i++ ) if ( ura_eph[i] >= value )
        {
            break;
        }
    return i;
    ...
    switch ( satsys )
    {
        ...
        case 5:
            *sys = SYS_QZS;
            break; /* OEM6 */
        default:
            fprintf( stderr, "oem4 unknown system: sys=%d\n", satsys );
        ...
        switch ( sigtype )
        {
            ...
            case  6:
                ...
                *code = CODE_L6I;
                break;
            default:
                fprintf( stderr, "oem4 unknown sigtype: sigtype=%d\n", sigtype ); //1.4.5 oem628e support
                freq = -1;
                break;
        }
    }
...
static int decode_bdsephemris( raw_t *raw )
{
    unsigned char *p = raw->buff + OEM4HLEN;
    //gtime_t tzero = {0};
    eph_t   beph = {0};
    ...
    beph.week = U4( p + 4 ) + BD2GPST_WEEK;
    //beph.sva = ura2index(R8(p+8));
    beph.sva = uraindex( R8( p + 8 ) ); //zy change
    beph.svh = U4( p + 16 );
    beph.tgd[0] = R8( p + 20 );
    beph.iodc = U4( p + 28 );
    beph.tocs = U4( p + 32 );
    ...
    beph.cis = R8( p + 180 );
    //-----------------------------------------------------------
    if ( beph.tocs != beph.toes )
    {
        return 0;
    }
    
    beph.toe = gpst2time( beph.week, beph.toes );  //zy add
    beph.toe = timeadd( beph.toe, 14 );           //zy add
    
    beph.toc = gpst2time( beph.week, beph.tocs );
    //  beph.toc = bdt2gpst(beph.toc);
    beph.tocs = time2gpst( beph.toc, &beph.week );
    beph.ttr = beph.toc;
    /*
    ...
    //  beph.toes += BD2GPST_SEC;
        beph.sat=sat;
        bool isFound = false;
        for(int i = 0; i < pBephData.GetCount(); i++)
        {
            eph_t& tmpeph = pBephData.GetAt(i);
            if( tmpeph.sat == beph.sat)
            {
                if(timediff(beph.toe, tzero) > 0.0 && (fabs(timediff(tmpeph.toe, beph.toe)) < 0.05))
                {
                    isFound = true;
                    break;
                }
                if(timediff(beph.toc, tzero) > 0.0 && (fabs(timediff(tmpeph.toe, beph.toe)) < 0.05))
                {
                    isFound = true;
                    break;
                }
        ...
        //pBephData.Add(beph);
        //bdsEph.eph_cn++;
        //bdsEph.ephem[prn-1]=beph;
    */
	if ( !strstr( raw->opt, "-EPHALL" ) )
	{
	//	if (beph.iode == raw->nav.eph[sat-1].iode)
	//	{
	//		logi("EPHALL BDS iode \n");
	//		 return (0); /* unchanged */
	//	}
		if ( fabs( timediff( beph.toe, raw->nav.eph[sat - 1].toe ) ) < 1E-6 )
		{
	//		logi("EPHALL toe iode \n");
			return ( 0 );	 //tya modified
		}
	//	if ( fabs( timediff( beph.toc, raw->nav.eph[sat - 1].toc ) ) < 1E-6 )
	//	{
	//		logi("EPHALL toc iode \n");
	//		return ( 0 );	 //tya modified
	//	}
	}
	//fprintf( stderr, " sat = %d, beph.toe.time= %d, beph.toc.time = %d \n", sat, (int)beph.toe.time, (int)beph.toc.time );


	beph.toes += BD2GPST_SEC;
    beph.sat = sat;
    raw->nav.eph[sat - 1] = beph;
    raw->ephsat = sat;
...
/* decode bdsephemerisb ------------------------------------------------------*/
static int decode_bdsephemris_oem6( raw_t *raw )
{
    unsigned char *p = raw->buff + OEM4HLEN;
    //gtime_t tzero = {0};
    eph_t   beph = {0};
 ...
 //   fprintf( stderr, "decode_bdsephemris_oem6\n");
    if ( msglen < 188 )
    {
        return -1;
    }
...
static int decode_oem4( raw_t *raw )
{
    ...
    switch ( type )
    {
        ...
        case ID_BDSEPHEMERIS:
             return decode_bdsephemris    ( raw );
        case ID_BDSEPHEMERIS_OEM6:
             return decode_bdsephemris_oem6( raw );
