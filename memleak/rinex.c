...
static void timestr_rnx( char *str )
{
	...
	time.sec = 0.0;
	time2epoch( time, ep );
	sprintf( str, "%04.0f%02.0f%02.0f %02.0f%02.0f%02.0f UTC", ep[ 0 ], ep[ 1 ],
			ep[ 2 ], ep[ 3 ], ep[ 4 ], ep[ 5 ] );
}
...
static void convcode( int ver, int sys, const char *str, char *type )
{
	...
	else if( ver ==3 && str[ 1 ] == 'A' ) /* ver.2.12 L1C/A */
	{
		if( sys == SYS_GPS )
		{
	...
	else if( ver ==3 && str[ 1 ] == 'B' ) /* ver.2.12 GPS L1C */
	{
		if( sys == SYS_GPS )
		{
	...
	else if( ver ==3 && str[ 1 ] == 'C' ) /* ver.2.12 GPS L2C */
	{
		if( sys == SYS_GPS )
		{
	...
	else if( ver ==3 && str[ 1 ] == 'D' ) /* ver.2.12 GLO L2C/A */
	{
		if( sys == SYS_GLO )
		{
	...
	else if( ver ==3 && str[ 1 ] == '1' ) /* ver.2.12 GPS L1PY,GLO L1P */
	{
		if( sys == SYS_GPS )
		{
	...
	else if( str[ 1 ] == '2' )
	{
		...
		{
			sprintf( type, "%c2C", str[ 0 ] ); /* tentative */
		}
	}
	else if( str[ 1 ] == '5' )
	{
		...
		{
			sprintf( type, "%c5X", str[ 0 ] ); /* tentative */
		}
	}
	else if( str[ 1 ] == '6' )
	{
		...
		{
			sprintf( type, "%c6X", str[ 0 ] ); /* tentative */
		}
	}
	else if( str[ 1 ] == '7' )
	{
		...
		{
			sprintf( type, "%c7X", str[ 0 ] ); /* tentative */
		}
	}
	else if( str[ 1 ] == '8' )
	{
	...
	trace( 3, "convcode: ver=%d sys=%2d type= %s -> %s\n", ver, sys, str,
			type );
}
/* decode obs header ---------------------------------------------------------*/
		...
		char tobs[ ][ MAXOBSTYPE ][ 4 ], nav_t *nav, sta_t *sta )
{
	...
	const char *defcodes[ ] = { "CWX   ", /* GPS: L125___ */
	"CC    ", /* GLO: L12____ */
	"X XXXX", /* GAL: L1_5678 */
	"CXXX  ", /* QZS: L1256__ */
	"C X   ", /* SBS: L1_5___ */
	...
	else if( strstr( label, "HUACE ANT PHASECENTER" ) )
		; //zy
	else if( strstr( label, "HUACE ANT REFERENCEPOINT" ) )
		; //zy
...
/* decode obs data -----------------------------------------------------------*/
static int decode_obsdata( FILE *fp, char *buff, int ver, sigind_t *index,
		obsd_t *obs )
{
	sigind_t *ind;
	...
	for( i = 0, j = ver ==2 ? 0 : 3; i < ind->n; i++, j += 16 )
	{
...
static int addobsdata( obs_t *obs, const obsd_t *data )
{
	...
	if( obs->nmax <= obs->n )
	{
		...
		if( !( obs_data = (obsd_t *)realloc( obs->data,
		...
		char tobs[ ][ MAXOBSTYPE ][ 4 ], int *flag, obsd_t *data )
{
	...
	while( fgets( buff, MAXRNXLEN, fp ) )
	{
		...
		if( i == 0 )
		{
			if( ( nsat = decode_obsepoch( fp, buff, ver, &time, flag, sats ) )
					<= 0 )
			{
				continue;
			}
		...
		else if( *flag <= 2 || *flag == 6 )
		{
			...
			/* decode obs data */
			if( decode_obsdata( fp, buff, ver, index, data + n ) && n < MAXOBS )
			{
				n++;
			}
		}
		...
		char tobs[ ][ MAXOBSTYPE ][ 4 ], obs_t *obs )
{
	...
	/* read rinex obs data body */
	while( ( n = readrnxobsb( fp, opt, ver, tobs, &flag, data ) ) >= 0
			&& stat >= 0 )
	{
		...
		geph_t *geph )
{
	...
	*geph = geph0;
	...
	tod = ver ==2 ? data[ 2 ] : fmod( data[ 2 ], 86400.0 ); /* tod (v.2), tow (v.3) in utc */
...
static int add_geph( nav_t *nav, const geph_t *geph )
{
	...
	{
		nav->ngmax += 1024;
		if( !( nav_geph = (geph_t *)realloc( nav->geph,
				sizeof(geph_t) * nav->ngmax ) ) )
		{
...
static int add_seph( nav_t *nav, const seph_t *seph )
{
	...
	{
		nav->nsmax += 1024;
		if( !( nav_seph = (seph_t *)realloc( nav->seph,
				sizeof(seph_t) * nav->nsmax ) ) )
		{
...
static int readrnxclk( FILE *fp, const char *opt, int index, nav_t *nav )
{
	...
	while( fgets( buff, sizeof( buff ), fp ) )
	{
		...
		{
			nav->ncmax += 1024;
			if( !( nav_pclk = (pclk_t *)realloc( nav->pclk,
					sizeof(pclk_t) * ( nav->ncmax ) ) ) )
			{
		...
		if( nav->nc <= 0
				|| fabs( timediff( time, nav->pclk[ nav->nc - 1 ].time ) )
						> 1E-9 )
		{
			nav->nc++;
...
static int readrnxfp( FILE *fp, gtime_t ts, gtime_t te, double tint,
		const char *opt, int flag, int index, char *type, obs_t *obs,
		nav_t *nav, sta_t *sta )
{
	int ver;
	int sys, tsys;
	...
	{
		case 'O':
			return readrnxobs( fp, ts, te, tint, opt, index, ver, tsys, tobs,
					obs );
		case 'N':
		...
		double tint, const char *opt, obs_t *obs, nav_t *nav, sta_t *sta )
{
	...
	{
		stat = readrnxfile( files[ i ], ts, te, tint, opt, 0, rcv, &type, obs,
				nav, sta );
	}
...
static void combpclk( nav_t *nav )
{
	...
	{
		if( fabs( timediff( nav->pclk[ i ].time, nav->pclk[ j ].time ) )
				< 1E-9 )
		{
			for( k = 0; k < MAXSAT; k++ )
			{
...
extern int readrnxc( const char *file, nav_t *nav )
{
	...
	for( i = 0; i < n; i++ )
	{
		if( readrnxfile( files[ i ], t, t, 0.0, NULL, 1, index++, &type, NULL,
				nav, NULL ) )
		{
			continue;
		}
...
extern int outrnxobsh( FILE *fp, const rnxopt_t *opt, const nav_t *nav )
{
	...
	fprintf( fp, "%-20.20s%-40.40s%-20s\n", opt->name[ 0 ], opt->name[ 1 ],
			"OBSERVER / AGENCY" );
	fprintf( fp, "%-20.20s%-20.20s%-20.20s%-20s\n", opt->rec[ 0 ],
			opt->rec[ 1 ], opt->rec[ 2 ], "REC # / TYPE / VERS" );
	fprintf( fp, "%-20.20s%-20.20s%-20.20s%-20s\n", opt->ant[ 0 ],
			opt->ant[ 1 ], opt->ant[ 2 ], "ANT # / TYPE" );
	...
	if( strstr( opt->rec[ 1 ], "CHC I80" ) || strstr( opt->rec[ 1 ], "CHC I70" )
			|| strstr( opt->rec[ 1 ], "CHC M6" )
			|| strstr( opt->rec[ 1 ], "CHC X10" ) )
	{
		fprintf( fp, "%-60.60s%-20s\n", "HUACE ANT PHASECENTER", "COMMENT" ); //zy
	}
	else if( strstr( opt->rec[ 1 ], "CHC N72" ) )
	{
		fprintf( fp, "%-60.60s%-20s\n", "HUACE ANT REFERENCEPOINT", "COMMENT" ); //zy
	}
	...
	if( opt->rnxver ==3 ) /* ver.3.02 */
	{
		...
		for( i = j = 0; i < ( n <= 0 ? 1 : ( n - 1 ) / 8 + 1 ); i++ )
		{
			...
			for( k = 0; k < 8; k++, j++ )
			{
				if( j < n )
				{
					fprintf( fp, " R%02d %2d", prn[ j ],
							nav->glo_fcn[ prn[ j ] - 1 ] - 8 );
				}
				else
				{
					fprintf( fp, " %6s", "" );
...
/**************************************************************************/
extern int outrnxmeteh_buf( FILE *fp, const rnxopt_t *opt, const mete_t *mete )
{
	double pos[ 3 ] = { 0 };
	int i = 0;
	char date[ 32 ];
	
	trace( 3, "outrnxmeteh_buf:\n" );
	...
	fprintf( fp, "%-20s%-20s      %7.1f    %-2s SENSOR MOD/TYPE/ACC \r\n",
			opt->metecompanyNa, opt->metesenserInfo[ 0 ], 0.0, "PR"
			//mete->senserInfo[0].senserTyp
			);
	fprintf( fp, "%-20s%-20s      %7.1f    %-2s SENSOR MOD/TYPE/ACC \r\n",
			opt->metecompanyNa, opt->metesenserInfo[ 1 ], 0.2, "TD"
			//mete->senserInfo[1].senserTyp
			);
	fprintf( fp, "%-20s%-20s      %7.1f    %-2s SENSOR MOD/TYPE/ACC \r\n",
			opt->metecompanyNa, opt->metesenserInfo[ 2 ], 0.2, "HR"
			//mete->senserInfo[2].senserTyp
			);
	...
	fprintf( fp, "%14.4f%14.4f%14.4f%14.4f %-2s SENSOR POS XYZ\r\n", pos[ 0 ],
			pos[ 1 ], pos[ 2 ], mete->H, "PR" );
	fprintf( fp, "%14.4f%14.4f%14.4f%14.4f %-2s SENSOR POS XYZ\r\n", pos[ 0 ],
			pos[ 1 ], pos[ 2 ], mete->H, "TD" );
	fprintf( fp, "%14.4f%14.4f%14.4f%14.4f %-2s SENSOR POS XYZ\r\n", pos[ 0 ],
			pos[ 1 ], pos[ 2 ], mete->H, "HR" );
	return fprintf( fp, "%-60.60s%-20s\n", "", "END OF HEADER" ) != EOF;
}

/**************************************************************************/
extern int outrnxmeteb_buf( rnx_dat_buf_t *dat_buf, const rnxopt_t *opt,
		const mete_t *mete_t, int n, int flag )
{
	double ep[ 6 ];

	dat_buf->dat_type = RNX_CONV_DAT_TYPE__METE;

	time2epoch( mete_t->time, ep );

	if( opt->rnxver ==2 ) /* ver.2 */
	{
		dat_buf->cur += sprintf( dat_buf->cur,
				" %02d %2d %2d %2d %2d %2d%7.1f%7.1f%7.1f\r\n",
				( (int)ep[ 0 ] ) % 100, (int)ep[ 1 ], (int)ep[ 2 ],
				(int)ep[ 3 ], (int)ep[ 4 ], (int)ep[ 5 ], mete_t->PR,
				mete_t->TD, mete_t->HR );
	}
	else /* ver.3 */
	{
		dat_buf->cur += sprintf( dat_buf->cur,
				" %04.0f %2d %2d %2d %2d %2d%7.1f%7.1f%7.1f\r\n", ep[ 0 ],
				(int)ep[ 1 ], (int)ep[ 2 ], (int)ep[ 3 ], (int)ep[ 4 ],
				(int)ep[ 5 ], mete_t->PR, mete_t->TD, mete_t->HR );
	}
	//fprintf( stderr, "[%s]\n", dat_buf->buf );
	return 1;
...
/**************************************************************************/
extern int outrnxobsh_buf( FILE *fp, const rnxopt_t *opt, const nav_t *nav )
{
	const char *glo_codes[ ] = { "C1C", "C1P", "C2C", "C2P" };
	double ep[ 6 ], pos[ 3 ] = { 0 }, del[ 3 ] = { 0 };
	int i;
	...
	{
		fprintf( fp, "%14.4s%14.4s%14.4s%-18s%-20s\n", "", "", "", "",
				"APPROX POSITION XYZ" );
	}
	else
	{
		fprintf( fp, "%14.4f%14.4f%14.4f%-18s%-20s\n", pos[ 0 ], pos[ 1 ],
				pos[ 2 ], "", "APPROX POSITION XYZ" );
	}
	fprintf( fp, "%14.4f%14.4f%14.4f%-18s%-20s\n", del[ 0 ], del[ 1 ], del[ 2 ],
			"", "ANTENNA: DELTA H/E/N" );
...
static void outrnxobsf( FILE *fp, double obs, int lli )
{
	...
	{
		obs = 0.0;
		fprintf( fp, "%14.3f", obs ); //tya modified
	}
	else
	{
...
// checked
/* output obs data field -----------------------------------------------------*/
static void outrnxobsf_buf( rnx_dat_buf_t *dat_buf, double obs, int lli )
{
	if( obs == 0.0 || obs <= -1E9 || obs >= 1E9 )
	{
		obs = 0.0;
		dat_buf->cur += sprintf( dat_buf->cur, "%14.3f", obs ); //tya modified
	}
	else
	{
		dat_buf->cur += sprintf( dat_buf->cur, "%14.3f", obs );
	}
	if( lli <= 0 )
	{
		dat_buf->cur += sprintf( dat_buf->cur, "  " );
	...
	{
		dat_buf->cur += sprintf( dat_buf->cur, "%1.1d ", lli );
	}
}

/* search obs data index -----------------------------------------------------*/
static int obsindex( int ver, int sys, const unsigned char *code,
		const char *tobs, const char *mask )
{
	...
	for( i = 0; i < NFREQ + NEXOBS; i++ )
	{
		...
		if( ver ==2 ) /* ver.2 */
		{
			...
			{
				if( code[ i ] == CODE_L1C || code[ i ] == CODE_L1I
						|| code[ i ] == CODE_L1Q || code[ i ] == CODE_L1X )
				{
					return i;
				}
			...
			{
				if( !strcmp( tobs, "C1" ) || !strcmp( tobs, "P1" )
						|| !strcmp( tobs, "L1" ) || !strcmp( tobs, "D1" )
						|| !strcmp( tobs, "S1" ) )
				{
					if( code[ i ] == CODE_L1I || code[ i ] == CODE_L1Q
							|| code[ i ] == CODE_L1X )
					{
						return i;
					}
				}
				else if( !strcmp( tobs, "C2" ) || !strcmp( tobs, "P2" )
						|| !strcmp( tobs, "L2" ) || !strcmp( tobs, "D2" )
						|| !strcmp( tobs, "S2" ) )
				{
					if( code[ i ] == CODE_L7I || code[ i ] == CODE_L7Q
							|| code[ i ] == CODE_L7X )
					{
						return i;
					}
				}
				else if( !strcmp( tobs, "C5" ) || !strcmp( tobs, "P5" )
						|| !strcmp( tobs, "L5" ) || !strcmp( tobs, "D5" )
						|| !strcmp( tobs, "S5" ) )
				{
					if( code[ i ] == CODE_L6I || code[ i ] == CODE_L6Q
							|| code[ i ] == CODE_L6X )
					{
						return i;
					}
			...
			{
				if( code[ i ] == CODE_L1P || code[ i ] == CODE_L1W
						|| code[ i ] == CODE_L1Y || code[ i ] == CODE_L1N )
				{
					return i;
				}
			...
			else if( !strcmp( tobs, "C2" )
					&& ( sys == SYS_GPS || sys == SYS_QZS ) )
			{
				if( code[ i ] == CODE_L2S || code[ i ] == CODE_L2L
						|| code[ i ] == CODE_L2X )
				{
					return i;
				}
			...
			{
				if( code[ i ] == CODE_L2P || code[ i ] == CODE_L2W
						|| code[ i ] == CODE_L2Y || code[ i ] == CODE_L2N
						|| code[ i ] == CODE_L2D || code[ i ] == CODE_L1I )
				{
					return i;
				}
			...
			else if( !strcmp( tobs, "L5" ) || !strcmp( tobs, "P5" )
					|| !strcmp( tobs, "S5" ) || !strcmp( tobs, "D5" ) )
			{
				if( code[ i ] == CODE_L6I )
				{
					return i;
				}
				if( code[ i ] == CODE_L5X )  //zy_add
				{
					return i;
				}
				/*id=code2obs(code[i],NULL);
			...
			else if( !strcmp( tobs, "L2" ) || !strcmp( tobs, "P2" )
					|| !strcmp( tobs, "S2" ) || !strcmp( tobs, "D2" ) )
			{
				if( code[ i ] == CODE_L1I || code[ i ] == CODE_L2P )
				{
					return i;
				}
			...
			{
				if( code[ i ] == CODE_L1S || code[ i ] == CODE_L1L
						|| code[ i ] == CODE_L1X )
				{
					return i;
				}
		...
		int flag )
{
	...
	for( i = ns = 0; i < n && ns < MAXOBS; i++ )
	{
		...
		if( !opt->nobs[ opt->rnxver ==2 ? 0 : s[ ns ] ] )
		{
			continue;
		}
		ind[ ns++ ] = i;
	...
	for( i = 0; i < ns; i++ )
	{
		...
		for( j = 0; j < opt->nobs[ m ]; j++ )
		{
			...
			/* search obs data index */
			if( ( k = obsindex( opt->rnxver, sys, obs[ ind[ i ] ].code,
					opt->tobs[ m ][ j ], mask ) ) < 0 )
			{
				outrnxobsf( fp, 0.0, -1 );
				continue;
			...
			switch( opt->tobs[ m ][ j ][ 0 ] )
			{
				...
				case 'P':
					...
					break;
				case 'L':
					outrnxobsf( fp, obs[ ind[ i ] ].L[ k ], -1 );
					break; //outrnxobsf(fp,obs[ind[i]].L[k],obs[ind[i]].LLI[k]); break;
				case 'D':
 ...
 * return : status (1:ok, 0:output error)
 *-----------------------------------------------------------------------------*/
extern int outrnxobsb_buf( rnx_dat_buf_t *dat_buf, const rnxopt_t *opt,
		const obsd_t *obs, int n, int flag )
{
	const char *mask;
	double ep[ 6 ];
	char sats[ MAXOBS ][ 4 ] = { "" };
	...
	{
		dat_buf->cur += sprintf( dat_buf->cur,
				" %02d %2.0f %2.0f %2.0f %2.0f%11.7f  %d%3d",
				(int)ep[ 0 ] % 100, ep[ 1 ], ep[ 2 ], ep[ 3 ], ep[ 4 ], ep[ 5 ],
				flag, ns );
		for( i = 0; i < ns; i++ )
		{
			...
			dat_buf->cur += sprintf( dat_buf->cur, "%-3s", sats[ i ] );
		}
	}
	else /* ver.3 */
	{
	...
	for( i = 0; i < ns; i++ )
	{
		...
		for( j = 0; j < opt->nobs[ m ]; j++ )
		{
			...
			switch( opt->tobs[ m ][ j ][ 0 ] )
			{
				case 'C':
				case 'P':
					outrnxobsf_buf( dat_buf, obs[ ind[ i ] ].P[ k ], -1 );
					break;
				case 'L':
					outrnxobsf_buf( dat_buf, obs[ ind[ i ] ].L[ k ], -1 );
					break;
				case 'D':
					outrnxobsf_buf( dat_buf, obs[ ind[ i ] ].D[ k ], -1 );
					break;
				case 'S':
					outrnxobsf_buf( dat_buf, obs[ ind[ i ] ].SNR[ k ] * 0.25,
							-1 );
					break;
			}
		}
...
{
	double e =
			fabs( value ) < 1E-99 ? 0.0 : floor( log10( fabs( value ) ) + 1.0 );
	...
	fprintf( fp, " %s%013.11fD%+03.0f", value < 0.0 ? "-" : " ",
			fabs( value ) / pow( 10.0, e - 1.0 ), e - 1.0 );
}

// checked
/* output nav member by rinex nav format -------------------------------------*/
static void outnavf_buf( rnx_dat_buf_t *dat_buf, double value )
{
	double e =
			fabs( value ) < 1E-99 ? 0.0 : floor( log10( fabs( value ) ) + 1.0 );
	//fprintf(fp," %s.%012.0fE%+03.0f",value<0.0?"-":" ",fabs(value)/pow(10.0,e-12.0),e);
	...
	dat_buf->cur += sprintf( dat_buf->cur, " %s%013.11fD%+03.0f",
			value < 0.0 ? "-" : " ", fabs( value ) / pow( 10.0, e - 1.0 ),
			e - 1.0 );
}
	...
	if( opt->rnxver ==2 ) /* ver.2 */
	{
		...
		if( opt->outtime )
		{
			...
			fprintf( fp, "%9.0f%9.0f %-20s\n", nav->utc_gps[ 2 ],
					nav->utc_gps[ 3 ], "DELTA-UTC: A0,A1,T,W" );
		}
	}
	else /* ver.3 */
	{
...
extern int outrnxcnavh( FILE *fp, const rnxopt_t *opt, const nav_t *nav )
{
	...
	{
		fprintf( fp, "%9.2f           %-20s%-20s%-20s\n", 2.11,
				"COMPASSNAV DATA", "", "RINEX VERSION / TYPE" );
	}
	else /* ver.3 */
	{
		fprintf( fp, "%9.2f           %-20s%-20s%-20s\n", 3.02,
				"N: GNSS NAV DATA", "C: COMPASS", "RINEX VERSION / TYPE" );
	}
	fprintf( fp, "%-20.20s%-20.20s%-20.20s%-20s\n", opt->prog, opt->runby, date,
			"PGM / RUN BY / DATE" );
...
extern int outrnxnavb( FILE *fp, const rnxopt_t *opt, const eph_t *eph )
{
	...
	if( opt->rnxver ==3 || sys == SYS_GAL || sys == SYS_CMP ) /* ver.3 or ver.2 GAL */
	{
		//tya modified
		if( sys == SYS_CMP && opt->rnxver ==2 )
		{
			fprintf( fp, "%02d %02d %2.0f %2.0f %2.0f %2.0f %4.1f", prn,
					(int)ep[ 0 ] % 100, ep[ 1 ], ep[ 2 ], ep[ 3 ], ep[ 4 ],
					ep[ 5 ] );
			sep = "   ";
		}
		else
		{
			...
			fprintf( fp, "%-3s %04.0f %2.0f %2.0f %2.0f %2.0f %2.0f", code,
 ...
 * return : status (1:ok, 0:output error)
 *-----------------------------------------------------------------------------*/
extern int outrnxnavb_buf( rnx_dat_buf_t *dat_buf, const rnxopt_t *opt,
		const eph_t *eph )
{
	double ep[ 6 ], ttr;
	int week, sys, prn;
	...
	if ( !( sys = satsys( eph->sat, &prn ) ) || !( sys & opt->navsys ) )
	{
		fprintf( stderr, "outrnxnavb_buf  err!" );
		return 0;
	}
#else
	if( !( sys = satsys( eph->sat, &prn ) ) )
	{
		fprintf( stderr, "outrnxnavb_buf  err! sys[%d] eph->sat[%d]\n", sys,
				eph->sat );
		return 0;
	}
	if( !( sys & opt->navsys ) )
	{
		fprintf( stderr, "outrnxnavb_buf  err!  sys[%d]navsys[%d]\n", sys,
				opt->navsys );
		return 0;
	}
	...
	if( opt->rnxver ==3 || sys == SYS_GAL || sys == SYS_CMP ) /* ver.3 or ver.2 GAL */
	{
		...
		{
			dat_buf->cur += sprintf( dat_buf->cur,
					"%02d %02d %2.0f %2.0f %2.0f %2.0f %4.1f", prn,
					(int)ep[ 0 ] % 100, ep[ 1 ], ep[ 2 ], ep[ 3 ], ep[ 4 ],
					ep[ 5 ] );
			sep = "   ";
		}
		else
		{
			...
			dat_buf->cur += sprintf( dat_buf->cur,
					"%-3s %04.0f %2.0f %2.0f %2.0f %2.0f %2.0f", code, ep[ 0 ],
	...
	else if( sys == SYS_QZS ) /* ver.2 or ver.3.02 QZS */
	{
		...
		dat_buf->cur += sprintf( dat_buf->cur,
				"%-3s %02d %2.0f %2.0f %2.0f %2.0f %4.1f", code,
				(int)ep[ 0 ] % 100, ep[ 1 ], ep[ 2 ], ep[ 3 ], ep[ 4 ],
				ep[ 5 ] );
		sep = "    ";
	}
	...
	outnavf_buf( dat_buf, eph->f0 );
	outnavf_buf( dat_buf, eph->f1 );
	outnavf_buf( dat_buf, eph->f2 );
	dat_buf->cur += sprintf( dat_buf->cur, "\n%s", sep );

	outnavf_buf( dat_buf, eph->iode ); /* GPS/QZS: IODE, GAL: IODnav, BDS: AODE */
	outnavf_buf( dat_buf, eph->crs );
	outnavf_buf( dat_buf, eph->deln );
	outnavf_buf( dat_buf, eph->M0 );
	dat_buf->cur += sprintf( dat_buf->cur, "\n%s", sep );

	outnavf_buf( dat_buf, eph->cuc );
	outnavf_buf( dat_buf, eph->e );
	outnavf_buf( dat_buf, eph->cus );
	outnavf_buf( dat_buf, sqrt( eph->A ) );
	dat_buf->cur += sprintf( dat_buf->cur, "\n%s", sep );

	outnavf_buf( dat_buf, eph->toes );
	outnavf_buf( dat_buf, eph->cic );
	outnavf_buf( dat_buf, eph->OMG0 );
	outnavf_buf( dat_buf, eph->cis );
	dat_buf->cur += sprintf( dat_buf->cur, "\n%s", sep );

	outnavf_buf( dat_buf, eph->i0 );
	outnavf_buf( dat_buf, eph->crc );
	outnavf_buf( dat_buf, eph->omg );
	outnavf_buf( dat_buf, eph->OMGd );
	dat_buf->cur += sprintf( dat_buf->cur, "\n%s", sep );

	outnavf_buf( dat_buf, eph->idot );
	outnavf_buf( dat_buf, eph->code );
	outnavf_buf( dat_buf, eph->week ); /* GPS/QZS: GPS week, GAL: GAL week, BDS: BDT week */
	outnavf_buf( dat_buf, eph->flag );
	dat_buf->cur += sprintf( dat_buf->cur, "\n%s", sep );

	outnavf_buf( dat_buf, uravalue( eph->sva ) );
	outnavf_buf( dat_buf, eph->svh );
	outnavf_buf( dat_buf, eph->tgd[ 0 ] ); /* GPS/QZS:TGD, GAL:BGD E5a/E1, BDS: TGD1 B1/B3 */
	if( sys == SYS_GAL || sys == SYS_CMP )
	{
		outnavf_buf( dat_buf, eph->tgd[ 1 ] ); /* GAL:BGD E5b/E1, BDS: TGD2 B2/B3 */
	}
	else
	{
		outnavf_buf( dat_buf, eph->iodc ); /* GPS/QZS:IODC */
	}
	dat_buf->cur += sprintf( dat_buf->cur, "\n%s", sep );

	ttr = time2gpst( eph->ttr, &week );
	outnavf_buf( dat_buf, ttr + ( week - eph->week ) * 604800.0 );

	if( sys == SYS_GPS || sys == SYS_QZS )
	{
		outnavf_buf( dat_buf, eph->fit );
	}
	else if( sys == SYS_CMP )
	{
		outnavf_buf( dat_buf, eph->iodc ); /* AODC */
	}
	else
	{
		outnavf_buf( dat_buf, 0.0 ); /* spare */
	}
	//tya add
	outnavf_buf( dat_buf, 0.0 ); //å¤ç¨
	outnavf_buf( dat_buf, 0.0 ); //å¤ç¨
	dat_buf->cur += sprintf( dat_buf->cur, "\n" );
	return 1;
}
 ...
 * return : status (1:ok, 0:output error)
 *-----------------------------------------------------------------------------*/
extern int outrnxgnavb_buf( rnx_dat_buf_t *dat_buf, const rnxopt_t *opt,
		const geph_t *geph )
{
	gtime_t toe;
	double ep[ 6 ], tof;
	...
	outnavf_buf( dat_buf, -geph->taun );
	outnavf_buf( dat_buf, geph->gamn );
	outnavf_buf( dat_buf, tof ); //çµæå¸§æ¶é´ tk
	dat_buf->cur += sprintf( dat_buf->cur, "\n%s", sep );

	outnavf_buf( dat_buf, geph->pos[ 0 ] / 1E3 );
	outnavf_buf( dat_buf, geph->vel[ 0 ] / 1E3 );
	outnavf_buf( dat_buf, geph->acc[ 0 ] / 1E3 );
	outnavf_buf( dat_buf, geph->svh );
	dat_buf->cur += sprintf( dat_buf->cur, "\n%s", sep );

	outnavf_buf( dat_buf, geph->pos[ 1 ] / 1E3 );
	outnavf_buf( dat_buf, geph->vel[ 1 ] / 1E3 );
	outnavf_buf( dat_buf, geph->acc[ 1 ] / 1E3 );
	outnavf_buf( dat_buf, geph->frq );
	dat_buf->cur += sprintf( dat_buf->cur, "\n%s", sep );

	outnavf_buf( dat_buf, geph->pos[ 2 ] / 1E3 );
	outnavf_buf( dat_buf, geph->vel[ 2 ] / 1E3 );
	outnavf_buf( dat_buf, geph->acc[ 2 ] / 1E3 );
	outnavf_buf( dat_buf, geph->age );
 ...
 * return : status (1:ok, 0:output error)
 *-----------------------------------------------------------------------------*/
extern int outrnxhnavb_buf( rnx_dat_buf_t *dat_buf, const rnxopt_t *opt,
		const seph_t *seph )
{
	double ep[ 6 ];
	int prn;
	...
	{
		dat_buf->cur += sprintf( dat_buf->cur,
				"%2d %02d %2.0f %2.0f %2.0f %2.0f %4.1f", prn - 100,
				(int)ep[ 0 ] % 100, ep[ 1 ], ep[ 2 ], ep[ 3 ], ep[ 4 ],
				ep[ 5 ] );
		sep = "   ";
	}
	...
	outnavf_buf( dat_buf, seph->af0 );
	outnavf_buf( dat_buf, seph->af1 );
	outnavf_buf( dat_buf, time2gpst( seph->tof, NULL ) );
	dat_buf->cur += sprintf( dat_buf->cur, "\n%s", sep );

	outnavf_buf( dat_buf, seph->pos[ 0 ] / 1E3 );
	outnavf_buf( dat_buf, seph->vel[ 0 ] / 1E3 );
	outnavf_buf( dat_buf, seph->acc[ 0 ] / 1E3 );
	outnavf_buf( dat_buf, seph->svh );
	dat_buf->cur += sprintf( dat_buf->cur, "\n%s", sep );

	outnavf_buf( dat_buf, seph->pos[ 1 ] / 1E3 );
	outnavf_buf( dat_buf, seph->vel[ 1 ] / 1E3 );
	outnavf_buf( dat_buf, seph->acc[ 1 ] / 1E3 );
	outnavf_buf( dat_buf, uravalue( seph->sva ) );
	dat_buf->cur += sprintf( dat_buf->cur, "\n%s", sep );

	outnavf_buf( dat_buf, seph->pos[ 2 ] / 1E3 );
	outnavf_buf( dat_buf, seph->vel[ 2 ] / 1E3 );
	outnavf_buf( dat_buf, seph->acc[ 2 ] / 1E3 );
	outnavf_buf( dat_buf, 0 );
 ...
 * return : status (1:ok, 0:output error)
 *-----------------------------------------------------------------------------*/
extern int outrnxhnavh_buf( rnx_dat_buf_t *dat_buf, const rnxopt_t *opt,
		const nav_t *nav )
{
	int i;
	char date[ 64 ];
	...
	dat_buf->cur += sprintf( dat_buf->cur, "%-20.20s%-20.20s%-20.20s%-20s\n",
	...
	for( i = 0; i < MAXCOMMENT; i++ )
	{
		...
		dat_buf->cur += sprintf( dat_buf->cur, "%-60.60s%-20s\n",
				opt->comment[ i ], "COMMENT" );
	}
	dat_buf->cur += sprintf( dat_buf->cur, "%60s%-20s\n", "", "END OF HEADER" );
	return 1;
}
...
extern int outrnxlnavh( FILE *fp, const rnxopt_t *opt, const nav_t *nav )
{
	...
	{
		fprintf( fp, "%9.2f           %-20s%-20s%-20s\n", 2.11,
				"GALILEO NAV DATA", "", "RINEX VERSION / TYPE" );
	}
	else /* ver.3 */
	{
		fprintf( fp, "%9.2f           %-20s%-20s%-20s\n", 3.02,
