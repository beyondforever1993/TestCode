...
static int decode_type_27( raw_t *raw, unsigned int rif, int endian );
...
//static int getTrimble_L1Code( BYTE obsType );
static long long HLBit2_6Int( unsigned char *str1 );
//static int getTrimble_L2Code( BYTE obsType );
static unsigned long HLBit2ULong( unsigned char *str1 );
static int decode_bds_ephemeris( raw_t *raw, int e );
static double HLBit2Double( unsigned char *str1 );
float HLBit2Float( unsigned char *str1 );
static int decode_glo_ephemeris( raw_t *raw, int e );
static unsigned short HLBit2Word( unsigned char *str1 );
...
static short HLBit2Short( unsigned char *str1 )
{
	int i;
	char str[ 10 ];
	short ret;
...
static long HLBit2Long( unsigned char *str1 )
{
	int i;
	char str[ 10 ];
	long ret;
...
extern int input_rt17( raw_t *raw, unsigned char data )
{
	...
	{
		status = decode_retsvdata( raw,
				strstr( raw->opt, "-LE" ) ? LITTLE_ENDIAN : BIG_ENDIAN );
		clear_packet_buffer( raw );
		return ( status );
	}
...
/* read_len : read out how many bytes */
extern int input_rt17_zy( raw_t *raw, unsigned char *data, int len, int *read_len )
{
	int status = 0;
	unsigned int page, pages, reply;
	unsigned char type;

	//fprintf(stderr, "=====input_rt17_zy \n");
	if( !data )
	{
		return 0;
	}
	...
	type = data[ 2 ];
	if( ( data[ 0 ] == STX )
			&& ( ( type == GENOUT ) || ( type == RAWDATA )
					|| ( type == RETSVDATA ) ) )
	{
		raw->plen = 4 + data[ 3 ] + 2; /* 4 (header) + length + 2 (trailer) */
		//fprintf( stderr, " raw->plen =  %d , len = %d \n", raw->plen, len );
	}
	memcpy( raw->pbuff, data, raw->plen );
	raw->pbyte = raw->plen;
	*read_len = raw->plen;

	//fprintf(stderr," raw->pbyte = %d, raw->nbyte  = %d\n", raw->pbyte, raw->nbyte );
	...
	if( raw->pbuff[ 2 ] == RETSVDATA )
	{
		status = decode_retsvdata( raw,strstr( raw->opt, "-LE" ) ? LITTLE_ENDIAN : BIG_ENDIAN );
		clear_packet_buffer( raw );
		return ( status );
	}
	if( raw->pbuff[ 2 ] == RAWDATA )
	{
		...
		pages = raw->pbuff[ 5 ] & 15;
		reply = raw->pbuff[ 6 ];
		//logd("666666 page:%d, pages:%d, reply:%d \n", page, pages, reply);
		if( raw->nbyte == 0 )
		{
			if( page != 1 )
			{
		...
		if( page == pages )
		{
			//logd("666666 start decode raw data... \n");
			status = decode_rawdata( raw,strstr( raw->opt, "-LE" ) ? LITTLE_ENDIAN : BIG_ENDIAN );
			clear_message_buffer( raw );
			return ( status );
		}
...
extern int input_rt17f( raw_t *raw, FILE *fp )
{
	...
	while( 1 )
	{
		...
		if( ( status = input_rt17( raw, (unsigned char)data ) ) )
		{
			return ( status );
		}
	}
...
static void decode_genout( raw_t *raw, int endian )
{
	...
	while( input_len )
	{
        ...
        /*
		 *logi(
		 *        "RT17: Packet type=0X40 (GENOUT), GSOF record type=%d (%s), " "length=%d.\n",
...
static int decode_glo_ephemeris( raw_t *raw, int e )
{
	...
	if( !strstr( raw->opt, "-EPHALL" ) )
	{
		...
		//	if ( fabs( timediff( geph.toc, raw->nav.geph[prn - 1].toc ) ) < 1E-6 )
		//    {
		//        return ( 0 );    //tya modified
		//    }
	}
...
static int decode_gal_ephemeris( raw_t *raw, int e )
{
	unsigned char *p = raw->pbuff;
	int prn, sat, toc, tow;
	//   unsigned int flags, toe;
    ...
    /*
	 *logi( "RT17: decode_gal_ephemeris, length=%d,%s\n", raw->plen, raw->opt );
     */
	...
	eph.week = HLBit2Short( p + 7 );
	eph.iodc = HLBit2Short( p + 13 );
	eph.iode = eph.iodc;
	tow = HLBit2Long( p + 9 );

	eph.tocs = HLBit2Long( p + 142 );
	toc = eph.tocs;
	eph.toes = HLBit2Long( p + 15 );
	eph.tgd[ 0 ] = HLBit2Double( p + 170 );
	eph.tgd[ 1 ] = HLBit2Double( p + 179 );
	eph.f2 = HLBit2Double( p + 162 );
	eph.f1 = HLBit2Double( p + 154 );
	eph.f0 = HLBit2Double( p + 146 );
	eph.crs = HLBit2Double( p + 19 ); //crs
	eph.deln = HLBit2Double( p + 27 ) * PI;
	eph.M0 = HLBit2Double( p + 35 ) * PI;
	eph.cuc = HLBit2Double( p + 43 ) * PI;
	eph.e = HLBit2Double( p + 51 );
	eph.cus = HLBit2Double( p + 59 ) * PI;
	sqrtA = HLBit2Double( p + 67 ); //sqrt_a
	eph.A = sqrtA * sqrtA;
	eph.cic = HLBit2Double( p + 75 ) * PI;
	eph.OMG0 = HLBit2Double( p + 83 ) * PI;
	eph.cis = HLBit2Double( p + 91 ) * PI;
	eph.i0 = HLBit2Double( p + 99 ) * PI;
	eph.crc = HLBit2Double( p + 107 );
	eph.omg = HLBit2Double( p + 115 ) * PI;
	eph.OMGd = HLBit2Double( p + 123 ) * PI;
	eph.idot = HLBit2Double( p + 131 ) * PI;
	//flag = HLBit2Long(raw+176); 
	//eph.flag = flag & 0x1;		
	eph.svh = HLBit2Short( p + 140 );
	eph.sva = p[ 139 ];
	//eph.fit = flag & 0x0400;	
	//-----------------------------------------------------------
	eph.week = adjgpsweek( eph.week );
...
static int decode_rawdata( raw_t *raw, int endian )
{
	...
	/*
	 logi(  "RT17: Packet type=0X57 (RAWDATA), recordtype=%d (%s), "
	...
	/*
	 | Process (or possibly ignore) the message.
	 */
	//logd("666666 recordtype:%d, rif:%d \n", recordtype, rif);
	switch( recordtype )
	{
		case 0:
		...
		case 7:
			...
			break;
		case 6:
			status = decode_type_27( raw, rif, endian );
			break;
		default:
			//logi( "RT17: Packet not processed.\n" );
			break;
	}
	//logd("666666 status:%d \n", status);
	return ( status );
}
...
static int decode_retsvdata( raw_t *raw, int endian )
{
	...
	/*
	 logi(  "RT17: packet type=0X55 (RETSVDATA), subtype=%d (%s), "
	...
	switch( subtype )
	{
		...
		case 3:
			...
			break;
		case 21:
			status = decode_bds_ephemeris( raw, endian );
			break;
		case 9:
			status = decode_glo_ephemeris( raw, endian );
			break;
		case 11:
			status = decode_gal_ephemeris( raw, endian );
			break;
		default:
            /*
			 *logi( "RT17: Packet not processed.\n" );
...
static int decode_type_17( raw_t *raw, unsigned int rif, int e )
{
	...
	receive_time = R8( p, e );
	p += 8; /* Receive time within the current GPS week (ms). */
	clock_offset = R8( p, e );
	p += 8; /* Clock offset value (ms). 0.0 = not known */
	...
	for( i = 0; ( i < nsat ) && ( i < MAXOBS ); i++ )
	{
		...
		l1_code =
				( flags2 & 1 ) ?
						( ( flags2 & 4 ) ? CODE_L1W : CODE_L1P ) : CODE_L1I;
		l2_code =
				( flags2 & 4 ) ?
						CODE_L2W : ( ( flags2 & 2 ) ? CODE_L7I : CODE_L2C );
#endif
...
static int getTrimble_L1Code( BYTE obsType )
{
	switch ( obsType )
	{
		...
		return CODE_L1P;
		case 2:
		return CODE_L1C;        //return CODE_L1X;
		case 9:
		return CODE_L1C;//return CODE_L1Y;
		case 10:
		return CODE_L1C;//return CODE_L1M;
		case 26:
		return CODE_L1C;//may be this is L1P
		case 27:
		return CODE_L1P;
	}
...
static int getTrimble_L2Code( BYTE obsType )
{
	...
	{
		case 0:
		return CODE_L2P;        //return CODE_L2C;
		case 1:
		return CODE_L2P;
		case 2:
		return CODE_L2P;//return CODE_L2D;
		case 3:
		return CODE_L2P;//return CODE_L2S;
		case 4:
		return CODE_L2P;//return CODE_L2L;
		case 5:
		return CODE_L2P;//return CODE_L2X;
		case 9:
		return CODE_L2P;//return CODE_L2Y;
		case 10:
		return CODE_L2P;//return CODE_L2M;
		case 28:
		return CODE_L2P;//return CODE_L2D;
		case 29:
		return CODE_L2P;
...
static int getTrimble_L5Code( BYTE obsType )
{
	switch ( obsType )
	{
		case 0:
...
static int decode_type_27( raw_t *raw, unsigned int rif, int e )
{
	unsigned char *p = raw->buff;
	//double            ep[6] = {0};
	gtime_t time;
	//int           j;
	unsigned char cEpochHeadLen = p[ 0 ];
	unsigned short wEpochLen = 0;
	BYTE Epoch_Flag = 0;
	BYTE Raim_Info = 0;
	double diff_time;
	int idx = 0;
	...
	unsigned char cMeasureHeadLen, cMeasureBlockLen;
	unsigned char cBlockType, cTrackType;
	int sys;
	//int sat;
	int prn;
	unsigned char cMeasureBlockNum;
	unsigned char nn = 0;
	unsigned char SlipCount1 = 0, MeasureFlag1 = 0, SlipCount2 = 0, MeasureFlag2 = 0;
	BYTE MeasureFlag3 = 0;
	BYTE Range_diff_overflow = 0;
	BYTE Time_Base = 0;
	BYTE Sv_Type = 0;
	char sv_Ch = 0;
	BYTE sv_f1 = 0, sv_f2 = 0, sv_f3 = 0;
	int index;

	if( cEpochHeadLen >= 12 && cEpochHeadLen <= 17 )
	{
		...
		if( Epoch_Flag & 0x02 )
		{
			diff_time = ( ( p[ 12 + idx ] << 16 ) + ( p[ 13 + idx ] << 8 )
					+ p[ 14 + idx ] ) / 4194304;
			idx += 3;
		}
		if( Epoch_Flag & 0x10 )
	...
	wEpochLen = cEpochHeadLen;
	week = adjgpsweek( week );
	time = gpst2time( week, sec );
	raw->time = time;
	//      TRACE("Raw2: %d  %.3f  %d\n ",ghtime.week,ghtime.sec,obs_d.n);
	if( n > MAXOBS || sec < 0 || sec > 604800 )
	{
		return 0;
	}
	//logd("666666 wEpochLen:%d, week:%d, time:%d \n", wEpochLen, week, time );

	if( Epoch_Flag & 0x20 )
	{
		Time_Base = p[ cEpochHeadLen + 1 ] & 0x0F;
		wEpochLen += p[ cEpochHeadLen ];
	}

	for( index = 0; index < n && index < MAXOBS; index++ )
	{
		cMeasureHeadLen = p[ wEpochLen ];
		if( cMeasureHeadLen < 8 || cMeasureHeadLen > 14 )
		{
			return 0;
		}

		prn = p[ wEpochLen + 1 ];
		Sv_Type = p[ wEpochLen + 2 ];
		if( Sv_Type == 0 )
		{
			sys = SYS_GPS;
		}
		...
		//logd("666666 prn:%d, sv_type:%d, sys:%d \n", prn, Sv_Type, sys);

		//tya add:�����ʷ��� 20150330
		memset( &raw->obs.data[ index ], 0, sizeof(obsd_t) );

		if( sys == SYS_CMP || sys == SYS_GLO || sys == SYS_GPS || sys == SYS_GAL
				|| sys == SYS_QZS || sys == SYS_SBS )
		{
			raw->obs.data[ index ].sat = (int)satno( sys, prn );
			//obs_d.data[jj].sat = satno(sys,prn);
		}
		else
		{
			raw->obs.data[ index ].sat = prn;
		}
		//logd("666666 raw->obs.data[ %d ].sat %d \n", index, raw->obs.data[ index ].sat);

		raw->obs.data[ index ].time = time;
		cMeasureBlockNum = p[ wEpochLen + 4 ];
		...
		if( sv_f2 & 0x80 )
		{
			sv_f3 = p[ wEpochLen + 8 + idx ];
			idx++;
		}
		wEpochLen += cMeasureHeadLen;

		//logd("666666 cMeasureBlockNum:%d \n",cMeasureBlockNum );
		//Ƶ��--------------fangzhiqiang 20160512
		for( nn = 0; nn < cMeasureBlockNum; nn++ )
		{
			cMeasureBlockLen = p[ wEpochLen ];

			if( cMeasureBlockLen < 15 && cMeasureBlockLen > 23 )
			{
				return 0;
			}
			if( sys != SYS_GPS && sys != SYS_GLO && sys != SYS_CMP
					&& sys != SYS_GAL )/*�ǿ���ϵͳ������������*/
			{
				wEpochLen += cMeasureBlockLen;
				continue;
			}
			cBlockType = p[ wEpochLen + 1 ];
			cTrackType = p[ wEpochLen + 2 ];

			//   if ( ( cBlockType == 0 && cTrackType == 0 )
			//           || ( cBlockType == 6 && cTrackType == 26 ) )

			//logi("666666 sys =%d ,blocktype=%d ,trackType=%d \n",sys,cBlockType,cTrackType);
			if( ( cBlockType == 0 &&
						( cTrackType == 0 || cTrackType == 1 || cTrackType == 2 || cTrackType == 9
								|| cTrackType == 10 || ( cTrackType >= 20 && cTrackType <= 25 ) ) )
					|| ( cBlockType == 6 && ( cTrackType == 26 || cTrackType == 27 ) ) )
			{
				raw->obs.data[ index ].SNR[ 0 ] = (BYTE)( ( ( p[ wEpochLen + 3 ]
						<< 8 ) + p[ wEpochLen + 4 ] ) / 10. + 0.5 ) * 4;
				if( sys != SYS_SBS && sys != SYS_QZS )
				{
					raw->obs.data[ index ].P[ 0 ] = HLBit2ULong(
							p + wEpochLen + 5 ) / 128.;
				}
				else
				{
				...
				raw->obs.data[ index ].L[ 0 ] = ( -1 )
						* HLBit2_6Int( p + wEpochLen + 9 ) / 32768.;

				//    raw->obs.data[index].code[0] = getTrimble_L1Code( cTrackType );
				//    logi(" code 1===%d\n",raw->obs.data[index].code[0]);
				raw->obs.data[ index ].D[ 0 ] = 0;

				if( sys == SYS_GPS )
				{
					raw->obs.data[ index ].code[ 0 ] = CODE_L1C;
				}
				else if( sys == SYS_GLO )
				{
					raw->obs.data[ index ].code[ 0 ] = CODE_L1C;
				...
				{
					raw->obs.data[ index ].code[ 0 ] = CODE_L1I;
				}
				else if( sys == SYS_GAL )
				{
					raw->obs.data[ index ].code[ 0 ] = CODE_L1X;
				}
				SlipCount1 = p[ wEpochLen + 15 ];
				MeasureFlag1 = p[ wEpochLen + 16 ];

				//------------------------------------
				idx = 0;
				if( MeasureFlag1 & 0x80 )
				{
					MeasureFlag2 = p[ wEpochLen + 17 ];
					if( MeasureFlag2 & 0x02 )
					{
						raw->obs.data[ index ].P[ 0 ] += RANGE_OVERFLOW_LIMIT0;
					}
					idx++;
				}
				if( MeasureFlag2 & 0x80 )
				{
					MeasureFlag3 = p[ wEpochLen + 17 + idx ];
					idx++;
				}
				if( MeasureFlag1 & 0x04 )
				{
					raw->obs.data[ index ].D[ 0 ] = (float)( ( ( p[ wEpochLen
							+ 17 + idx ] << 16 )
							+ ( p[ wEpochLen + 18 + idx ] << 8 )
							+ p[ wEpochLen + 19 + idx ] ) / 256. );
					idx += 3;
				}
				if( MeasureFlag2 & 0x01 )
				{
					Range_diff_overflow = p[ wEpochLen + 17 + idx ];
				}
				if( Sv_Type == 7 )
				{
					if( prn <= 5 )
					{
						raw->obs.data[ index ].L[ 0 ] += 0.5; //BD GEO 1~5 B1 observation fix
					}
				}
			}
			//else if ( ( cBlockType == 0 && cTrackType == 1 )
			//          || ( cBlockType == 6 && cTrackType == 27 ) )
			//{
			//}
			//else if ( cBlockType == 1 || cBlockType == 3 )
			//L2/B2/E5A
			else if( ( cBlockType == 1
					&& ( ( cTrackType >= 0 && cTrackType <= 5 )
							|| cTrackType == 9 || cTrackType == 10 ) )
					|| ( cBlockType == 3
							&& ( /*cTrackType == 13 || */cTrackType == 28 ) )
					|| ( cBlockType == 2
							&& ( cTrackType == 11 || cTrackType == 12
									|| cTrackType == 13 ) ) )
			{

				raw->obs.data[ index ].SNR[ 1 ] = (BYTE)( ( ( p[ wEpochLen + 3 ]
						<< 8 ) + p[ wEpochLen + 4 ] ) / 10. + 0.5 ) * 4;
				raw->obs.data[ index ].P[ 1 ] = HLBit2Short( p + wEpochLen + 5 )
						/ 256. + raw->obs.data[ index ].P[ 0 ];
				raw->obs.data[ index ].L[ 1 ] = ( -1 )
						* HLBit2_6Int( p + wEpochLen + 7 ) / 32768.;
				if( Sv_Type == 0
						&& ( cTrackType == 3 || cTrackType == 4
								|| cTrackType == 5 ) )
				{
					raw->obs.data[ index ].L[ 1 ] -= 0.25;
				}
				//     raw->obs.data[index].code[1] = getTrimble_L2Code( cTrackType );
				//	logi(" code 2===%d\n",raw->obs.data[index].code[1]);

				if( sys == SYS_GPS )
				{
					raw->obs.data[ index ].code[ 1 ] = CODE_L2P;
				}
				else if( sys == SYS_GLO )
				{
					raw->obs.data[ index ].code[ 1 ] = CODE_L2P;
				...
				{
					raw->obs.data[ index ].code[ 1 ] = CODE_L7I;
				}
				else if( sys == SYS_GAL )
				{
					raw->obs.data[ index ].code[ 1 ] = CODE_L5X;
				}
				SlipCount2 = p[ wEpochLen + 13 ];
				MeasureFlag1 = p[ wEpochLen + 14 ];

				idx = 0;
				if( MeasureFlag1 & 0x80 )
				{
					MeasureFlag2 = p[ wEpochLen + 15 ];
					idx++;
				}
				if( MeasureFlag2 & 0x80 )
				{
					MeasureFlag3 = p[ wEpochLen + 15 + idx ];
					idx++;
				}
				if( MeasureFlag1 & 0x04 )
				{
					raw->obs.data[ index ].D[ 1 ] = (float)( ( ( p[ wEpochLen
							+ 15 + idx ] << 16 )
							+ ( p[ wEpochLen + 16 + idx ] << 8 )
							+ p[ wEpochLen + 17 + idx ] ) / 256. );
					idx += 3;
				}
				if( MeasureFlag2 & 0x01 )
				{
					Range_diff_overflow = p[ wEpochLen + 15 + idx ];
				}
			}
			//   else if( cBlockType == 2 || cBlockType == 7 ||  cBlockType == 9 )
			//L5/B3/E5B
			else if( ( cBlockType == 2
					&& ( cTrackType == 6 || cTrackType == 7 || cTrackType == 8 ) )
					|| ( cBlockType == 7 && cTrackType == 29 )
					|| ( cBlockType == 9
							&& ( cTrackType == 32 || cTrackType == 33
									|| cTrackType == 34 ) )
					|| ( cBlockType == 3
							&& ( cTrackType == 11 || cTrackType == 12
									|| cTrackType == 13 ) ) )
			{
				...
				raw->obs.data[ index ].SNR[ 2 ] = (BYTE)( ( ( p[ wEpochLen + 3 ]
						<< 8 ) + p[ wEpochLen + 4 ] ) / 10. + 0.5 ) * 4;
				raw->obs.data[ index ].P[ 2 ] = HLBit2Short( p + wEpochLen + 5 )
						/ 256. + raw->obs.data[ index ].P[ 0 ];
				raw->obs.data[ index ].L[ 2 ] = ( -1 )
						* HLBit2_6Int( p + wEpochLen + 7 ) / 32768.;
				//if ( Sv_Type == 0 && ( cTrackType == 3 || cTrackType == 4 || cTrackType == 5 ) )
				//{
				//    raw->obs.data[index].L[1] -= 0.25;
				//}
				//    raw->obs.data[index].code[2] = getTrimble_L5Code( cTrackType );
				//	logi(" code 3===%d\n",raw->obs.data[index].code[2]);

				raw->obs.data[ index ].D[ 2 ] = 0;

				if( sys == SYS_GPS )
				{
					raw->obs.data[ index ].code[ 2 ] = CODE_L5X;
				}
				else if( sys == SYS_GLO )
				{
					raw->obs.data[ index ].code[ 2 ] = CODE_L3X;
				}
				else if( sys == SYS_CMP )
				{
					raw->obs.data[ index ].code[ 2 ] = CODE_L6I;
				}
				else if( sys == SYS_GAL )
				{
					raw->obs.data[ index ].code[ 2 ] = CODE_L7X;
				}
				SlipCount1 = p[ wEpochLen + 13 ];
				MeasureFlag1 = p[ wEpochLen + 14 ];
				...
				if( MeasureFlag1 & 0x04 )
				{
					raw->obs.data[ index ].D[ 2 ] = (float)( ( ( p[ wEpochLen
							+ 15 + idx ] << 16 )
							+ ( p[ wEpochLen + 16 + idx ] << 8 )
							+ p[ wEpochLen + 17 + idx ] ) / 256. );
					idx += 3;
				...
				//fprintf( stderr, " cBlockType = %d, cTrackType = %d, SNR[%lf]P[%lf]L[%lf]D[%lf]\n", cBlockType, cTrackType, raw->obs.data[index].SNR[2]* 1.0, raw->obs.data[index].P[2], raw->obs.data[index].L[2],raw->obs.data[index].D[2]  );
			}
			//E5(A+B)
			else if( cBlockType == 4
					&& ( cTrackType >= 14 && cTrackType <= 17 ) )
			{
				raw->obs.data[ index ].SNR[ 3 ] = (BYTE)( ( ( p[ wEpochLen + 3 ]
						<< 8 ) + p[ wEpochLen + 4 ] ) / 10. + 0.5 ) * 4;
				raw->obs.data[ index ].P[ 3 ] = HLBit2Short( p + wEpochLen + 5 )
						/ 256. + raw->obs.data[ index ].P[ 0 ];
				raw->obs.data[ index ].L[ 3 ] = ( -1 )
						* HLBit2_6Int( p + wEpochLen + 7 ) / 32768.;
				//	raw->obs.data[index].code[3] = getTrimble_L5Code( cTrackType );
				raw->obs.data[ index ].D[ 3 ] = 0;
				if( sys == SYS_GAL )
				{
					raw->obs.data[ index ].code[ 3 ] = CODE_L8X;
					//    logi(" GAL code 4\n");
				}
				...
				if( MeasureFlag1 & 0x04 )
				{
					raw->obs.data[ index ].D[ 3 ] = (float)( ( ( p[ wEpochLen
							+ 15 + idx ] << 16 )
							+ ( p[ wEpochLen + 16 + idx ] << 8 )
							+ p[ wEpochLen + 17 + idx ] ) / 256. );
					idx += 3;
			...
			//E6
			else if( cBlockType == 5 )
			{
				raw->obs.data[ index ].SNR[ 4 ] = (BYTE)( ( ( p[ wEpochLen + 3 ]
						<< 8 ) + p[ wEpochLen + 4 ] ) / 10. + 0.5 ) * 4;
				raw->obs.data[ index ].P[ 4 ] = HLBit2Short( p + wEpochLen + 5 )
						/ 256. + raw->obs.data[ index ].P[ 0 ];
				raw->obs.data[ index ].L[ 4 ] = ( -1 )
						* HLBit2_6Int( p + wEpochLen + 7 ) / 32768.;
				//	raw->obs.data[index].code[3] = getTrimble_L5Code( cTrackType );
				raw->obs.data[ index ].D[ 4 ] = 0;
				if( sys == SYS_GAL )
				{
					raw->obs.data[ index ].code[ 4 ] = CODE_L6X;
					//    logi(" GAL code 5\n");
				}
				...
				if( MeasureFlag1 & 0x04 )
				{
					raw->obs.data[ index ].D[ 4 ] = (float)( ( ( p[ wEpochLen
							+ 15 + idx ] << 16 )
							+ ( p[ wEpochLen + 16 + idx ] << 8 )
							+ p[ wEpochLen + 17 + idx ] ) / 256. );
					idx += 3;
...
static int decode_type_29( raw_t *raw, int endian )
{
	...
	if( *p < 3 )
		trace( 2,
				"RT17: Enhanced Position record block #1 length %d < 3 bytes. "
						"Record discarded.\n", *p );
	else
	{
...
static void unwrap_rawdata( raw_t *raw, unsigned int *rif )
{
	...
	*rif = p_in[ 7 ];
	//logd("666666 length_in_total:%d, rif:%d \n", length_in_total, *rif);
	while( length_in_total > 0 )
	{
	...
	//logd("666666 length_out_total:%d \n", length_out_total);
	raw->nbyte = raw->len = length_out_total;
}
