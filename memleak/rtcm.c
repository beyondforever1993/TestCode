...
#define CHCRTCM3PREAMB 0x79        /* rtcm ver.3 frame preamble */
...
extern int init_rtcm( rtcm_t *rtcm )
{
	...
	rtcm->sta.rectype[ 0 ] = rtcm->sta.recver[ 0 ] = rtcm->sta.recsno[ 0 ] =
			'\0';
	...
	if( !( rtcm->obs.data = (obsd_t *)malloc( sizeof(obsd_t) * MAXOBS ) )
			...
			|| !( rtcm->nav.geph = (geph_t *)malloc(
					sizeof(geph_t) * MAXPRNGLO ) ) )
	{
		free_rtcm( rtcm );
		return 0;
	}
...
extern int gen_rtcm3( rtcm_t *rtcm, int type, int sync )
{
	...
	setbitu( rtcm->buff, i, 8, CHCRTCM3PREAMB );
	i += 8;
