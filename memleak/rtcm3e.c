...
"", "1C", "1A", "1B", "1X", "1Z", "", "6C", "6A", "6B", "6X", "6Z", "", "7I",
		"7Q", "7X", "", "8I", "8Q", "8X", "", "5I", "5Q", "5X", "", "", "", "",
		"", "", "", "" };
...
"", "1I", "1Q", "1X", "", "", "", "6I", "6Q", "6X", "", "", "", "7I", "7Q",
		"7X", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "" };

/* ssr update intervals ------------------------------------------------------*/
static const double ssrudint[ 16 ] = { 1, 2, 5, 10, 15, 30, 60, 120, 240, 300,
...
static void gen_obs_gps( rtcm_t *rtcm, const obsd_t *data, int *code1, int *pr1,
		int *ppr1, int *lock1, int *amb, int *cnr1, int *code2, int *pr21,
		int *ppr2, int *lock2, int *cnr2 )
{
	...
	*pr1 = *amb = 0;
	...
	/* L2 -L1 pseudorange */
	if( data->P[ 0 ] != 0.0 && data->P[ 1 ] != 0.0 && data->code[ 0 ]
			&& data->code[ 1 ] && fabs( data->P[ 1 ] - pr1c ) <= 163.82 )
	{
		if( pr21 )
		{
	...
	if( data->P[ 0 ] != 0.0 && data->L[ 1 ] != 0.0 && data->code[ 0 ]
			&& data->code[ 1 ] )
	{
	...
	lt2 = locktime( data->time, rtcm->lltime[ data->sat - 1 ] + 1,
			data->LLI[ 1 ] );
...
/* generate obs field data compass -----------------------------------------------*/
static void gen_obs_compass( rtcm_t *rtcm, const obsd_t *data, int *code1,
		int *pr1, int *ppr1, int *lock1, int *amb1, int *amb2, int *amb3,
		int *cnr1, int *code2, int *pr2, int *ppr2, int *lock2, int *cnr2,
		int *code3, int *pr3, int *ppr3, int *lock3, int *cnr3 )
{
	double lam1, lam2, lam3, pr1c = 0.0, pr2c = 0.0, pr3c = 0.0;
	int lt1, lt2, lt3;

	lam1 = CLIGHT / FREQ2_CMP;
	lam2 = CLIGHT / FREQ7_CMP;
	lam3 = CLIGHT / FREQ6_CMP;
	*pr1 = *amb1 = 0;
	*pr2 = *amb2 = 0;
	*pr3 = *amb3 = 0;
	if( ppr1 )
	...
	if( ppr3 )
	{
		*ppr3 = 0xFFF80000; /* invalid values */
	}

	/* L1 peudorange */
	if( data->P[ 0 ] != 0.0 && data->code[ 0 ] )
	{
		*amb1 = (int)floor( data->P[ 0 ] / PRUNIT_GPS );
		*pr1 = ROUND( ( data->P[0] - *amb1 * PRUNIT_GPS ) / 0.02 );
		pr1c = *pr1 * 0.02 + *amb1 * PRUNIT_GPS;
	}
	/* L1 phaserange */
	if( data->P[ 0 ] != 0.0 && data->L[ 0 ] != 0.0 && data->code[ 0 ] )
	{
		...
		{
			*ppr1 = (long)ROUND(
					2000 * ( ( ( data->L[ 0 ] ) * lam1 ) - pr1c ) );
		}
	}
	/* L2 peudorange */
	if( data->P[ 1 ] != 0.0 && data->code[ 1 ] )
	{
		*amb2 = (int)floor( data->P[ 1 ] / PRUNIT_GPS );
		*pr2 = ROUND( ( data->P[1] - *amb2 * PRUNIT_GPS ) / 0.02 );
		pr2c = *pr2 * 0.02 + *amb2 * PRUNIT_GPS;
	}
	/* L2 phaserange */
	if( data->P[ 1 ] != 0.0 && data->L[ 1 ] != 0.0 && data->code[ 1 ] )
	{
		if( ppr2 )
		{
			*ppr2 = (long)ROUND(
					2000 * ( ( ( data->L[ 1 ] ) * lam2 ) - pr2c ) );
		}
	}
	/* L3 peudorange */
	if( data->P[ 2 ] != 0.0 && data->code[ 2 ] )
	{
		*amb3 = (int)floor( data->P[ 2 ] / PRUNIT_GPS );
		*pr3 = ROUND( ( data->P[2] - *amb3 * PRUNIT_GPS ) / 0.02 );
		pr3c = *pr3 * 0.02 + *amb3 * PRUNIT_GPS;
	}
	/* L3 phaserange */
	if( data->P[ 2 ] != 0.0 && data->L[ 2 ] != 0.0 && data->code[ 2 ] )
	{
		if( ppr3 )
		{
			*ppr3 = (long)ROUND(
					2000 * ( ( ( data->L[ 2 ] ) * lam3 ) - pr3c ) );
		}
	}
	lt1 = locktime( data->time, rtcm->lltime[ data->sat - 1 ], data->LLI[ 0 ] );
	lt2 = locktime( data->time, rtcm->lltime[ data->sat - 1 ] + 1,
			data->LLI[ 1 ] );
	lt3 = locktime( data->time, rtcm->lltime[ data->sat - 1 ] + 2,
			data->LLI[ 2 ] );
...
static int encode_type1001( rtcm_t *rtcm, int sync )
{
	...
	for( j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++ )
	{
		...
		/* generate obs field data gps */
		gen_obs_gps( rtcm, rtcm->obs.data + j, &code1, &pr1, &ppr1, &lock1,
				&amb, NULL, NULL, NULL, NULL, NULL, NULL );
...
static int encode_type1002( rtcm_t *rtcm, int sync )
{
	...
	for( j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++ )
	{
		...
		/* generate obs field data gps */
		gen_obs_gps( rtcm, rtcm->obs.data + j, &code1, &pr1, &ppr1, &lock1,
				&amb, &cnr1, NULL, NULL, NULL, NULL, NULL );
...
static int encode_type1003( rtcm_t *rtcm, int sync )
{
	...
	for( j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++ )
	{
		...
		/* generate obs field data gps */
		gen_obs_gps( rtcm, rtcm->obs.data + j, &code1, &pr1, &ppr1, &lock1,
				&amb, NULL, &code2, &pr21, &ppr2, &lock2, NULL );
...
static int encode_type1004( rtcm_t *rtcm, int sync )
{
	...
	for( j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++ )
	{
		...
		/* generate obs field data gps */
		gen_obs_gps( rtcm, rtcm->obs.data + j, &code1, &pr1, &ppr1, &lock1,
				&amb, &cnr1, &code2, &pr21, &ppr2, &lock2, &cnr2 );
...
/* encode type 1104: extended L1&L2&L5 compass rtk observables ----------------------*/
static int encode_type1104( rtcm_t *rtcm, int sync )
{
	//

	//**********************************************************//
	int i, j, nsat = 0, sys, prn;
	int code1, pr1, ppr1, lock1, amb1, amb2, amb3, cnr1, code2, pr21, ppr2,
			lock2, cnr2, code3, pr31, ppr3, lock3, cnr3;
	int BD2B1B2B3_Indicator = 6;
	char B3Idr = 0;
	int FreNum = 2;
	//DoNothing( &FreNum ); //to get rid of warning
	trace( 3, "encode_type1104: sync=%d\n", sync );
	...
	if( B3Idr )
	{
		BD2B1B2B3_Indicator = 7;
		FreNum = 3;
	}
	/* encode header */
	i = encode_head( 1104, rtcm, SYS_CMP, sync, nsat );
	setbitu( rtcm->buff, i, 3, BD2B1B2B3_Indicator );
	i += 3;
	//***************************//
	for( j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++ )
	{
		...
		//obsd_t *data=&rtcm->obs.data[j];
		//**************************************//
		/* generate obs field data gps */
		gen_obs_compass( rtcm, rtcm->obs.data + j, &code1, &pr1, &ppr1, &lock1,
				&amb1, &amb2, &amb3, &cnr1, &code2, &pr21, &ppr2, &lock2, &cnr2,
				&code3, &pr31, &ppr3, &lock3, &cnr3 );

		setbitu( rtcm->buff, i, 6, prn );
		i += 6;
		//B1
		setbitu( rtcm->buff, i, 2, code1 );
		i += 2;
		setbitu( rtcm->buff, i, 24, pr1 );
		i += 24;
		setbits( rtcm->buff, i, 20, ppr1 );
		...
		setbitu( rtcm->buff, i, 2, code2 );
		i += 2;
		setbits( rtcm->buff, i, 24, pr21 );
		i += 24;
		setbits( rtcm->buff, i, 20, ppr2 );
		i += 20;
		setbitu( rtcm->buff, i, 7, lock2 );
		i += 7;
		setbitu( rtcm->buff, i, 8, amb2 );
		i += 8;
		setbitu( rtcm->buff, i, 8, cnr2 );
		i += 8;
		if( B3Idr )
		{
			//B3
			setbitu( rtcm->buff, i, 2, code3 );
			i += 2;
			setbits( rtcm->buff, i, 24, pr31 );
			i += 24;
			setbits( rtcm->buff, i, 20, ppr3 );
			i += 20;
			setbitu( rtcm->buff, i, 7, lock3 );
			i += 7;
			setbitu( rtcm->buff, i, 8, amb3 );
			i += 8;
			setbitu( rtcm->buff, i, 8, cnr3 );
			i += 8;
		}
	}
	rtcm->nbit = i;
...
static int encode_type1009( rtcm_t *rtcm, int sync )
{
	...
	for( j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++ )
	{
		...
		/* generate obs field data glonass */
		gen_obs_glo( rtcm, rtcm->obs.data + j, fcn, &code1, &pr1, &ppr1, &lock1,
				&amb, NULL, NULL, NULL, NULL, NULL, NULL );
...
static int encode_type1020( rtcm_t *rtcm, int sync )
{
	...
	gtime_t time;
	double ep[ 6 ];
	int i = 24, j, prn, tk_h, tk_m, tk_s, tb, pos[ 3 ], vel[ 3 ], acc[ 3 ],
			gamn, taun, dtaun;
	int fcn, NT;
	...
	i += 5;
	setbitu( rtcm->buff, i, 1, 1 );
	i += 1; /* almanac health,Cn word */        //lzt modified
	setbitu( rtcm->buff, i, 1, 0 );
	i += 1; /* almanac health,P1 */
	setbitu( rtcm->buff, i, 2, 0 );
	i += 2; /* almanac health,P1 */
		...
		int iod, double udint, int refd, int provid, int solid )
{
	...
	trace( 4,
			"encode_ssr_head: type=%d sys=%d nsat=%d sync=%d iod=%d udint=%.0f\n",
	...
	i += 4; /* solution id */
	setbitu( rtcm->buff, i, np, nsat );
	i += np; /* no of satellites */
	return i;
}
...
{
	double udint = 0.0;
	int i, j, iod = 0, nsat, prn, iode, refd = 0, np, offp, deph[ 3 ],
			ddeph[ 3 ];
...
static int encode_ssr2( rtcm_t *rtcm, int sys, int sync )
{
	...
	for( j = 0; j < MAXSAT; j++ )
	{
		...
		iode = rtcm->ssr[ j ].iode;
		//DoNothing( &iode ); //to get rid of warning
...
{
	const int codes_gps[ ] = {
	CODE_L1C, CODE_L1P, CODE_L1W, CODE_L1Y, CODE_L1M, CODE_L2C, CODE_L2D,
			CODE_L2S,
			CODE_L2L, CODE_L2X, CODE_L2P, CODE_L2W, CODE_L2Y, CODE_L2M,
			CODE_L5I, CODE_L5Q,
			CODE_L5X };
	const int codes_glo[ ] = {
	CODE_L1C, CODE_L1P, CODE_L2C, CODE_L2P };
	const int codes_gal[ ] = {
	CODE_L1A, CODE_L1B, CODE_L1C, CODE_L1X, CODE_L1Z, CODE_L5I, CODE_L5Q,
			CODE_L5X,
			CODE_L7I, CODE_L7Q, CODE_L7X, CODE_L8I, CODE_L8Q, CODE_L8X,
			CODE_L6A, CODE_L6B,
			CODE_L6C, CODE_L6X, CODE_L6Z };
	const int codes_qzs[ ] = {
	CODE_L1C, CODE_L1S, CODE_L1L, CODE_L2S, CODE_L2L, CODE_L2X, CODE_L5I,
			CODE_L5Q,
			CODE_L5X };
	const int *codes;
	double udint = 0.0;
	...
	for( j = nsat = 0; j < MAXSAT; j++ )
	{
		...
		for( k = nbias = 0; k < ncode; k++ )
		{
			...
			code[ nbias ] = k;
			bias[ nbias++ ] = ROUND(
					rtcm->ssr[ j ].cbias[ codes[ k ] - 1 ] / 0.01 );
		}
		setbitu( rtcm->buff, i, np, prn - offp );
		i += np; /* satellite id */
...
static void gen_msm_index( rtcm_t *rtcm, int sys, int *nsat, int *nsig,
		int *ncell, unsigned char *sat_ind, unsigned char *sig_ind,
		unsigned char *cell_ind )
{
	...
	*nsat = *nsig = *ncell = 0;
	...
	for( i = 0; i < rtcm->obs.n; i++ )
	{
		...
		for( j = 0; j < NFREQ + NEXOBS; j++ )
		{
			...
			cell = sig_ind[ sig - 1 ] - 1
					+ ( sat_ind[ sat - 1 ] - 1 ) * ( *nsig );
			cell_ind[ cell ] = 1;
		}
	}
...
static void gen_msm_sat( rtcm_t *rtcm, int sys, int nsat,
		const unsigned char *sat_ind, double *rrng, double *rrate,
		unsigned char *info )
{
	obsd_t *data;
	...
	for( i = 0; i < rtcm->obs.n; i++ )
	{
		...
		for( j = 0; j < NFREQ + NEXOBS; j++ )
		{
			...
			//info[k]=sys!=SYS_GLO?0:(fcn<0?15:fcn);  /*Luv*/
		}
	}
}
/* generate msm signal data fields -------------------------------------------*/
static void gen_msm_sig( rtcm_t *rtcm, int sys, int nsat, int nsig, int ncell,
		const unsigned char *sat_ind, const unsigned char *sig_ind,
		const unsigned char *cell_ind, const double *rrng, const double *rrate,
		double *psrng, double *phrng, double *rate, int *lock,
		unsigned char *half, float *cnr )
{
	...
	for( i = 0; i < rtcm->obs.n; i++ )
	{
		...
		for( j = 0; j < NFREQ + NEXOBS; j++ )
		{
			...
			phrng_s =
					data->L[ j ] == 0.0 || lambda <= 0.0 ?
							0.0 : data->L[ j ] * lambda - rrng[ k ];
			rate_s =
					data->D[ j ] == 0.0 || lambda <= 0.0 ?
							0.0 : -data->D[ j ] * lambda - rrate[ k ];

			/* subtract phase - psudorange integer cycle offset */
			LLI = data->LLI[ j ];
			if( ( LLI & 1 )
					|| fabs( phrng_s - rtcm->cp[ data->sat - 1 ][ j ] )
							> 1171.0 )
			{
				rtcm->cp[ data->sat - 1 ][ j ] = ROUND( phrng_s / lambda )
						* lambda;
				LLI |= 1;
			}
...
/* encode msm header ---------------------------------------------------------*/
static int encode_msm_head( int type, rtcm_t *rtcm, int sys, int sync,
		int *nsat, int *ncell, double *rrng, double *rrate, unsigned char *info,
		double *psrng, double *phrng, double *rate, int *lock,
		unsigned char *half, float *cnr )
{
	double tow;
	...
	if( sys == SYS_GLO )
	{
		...
		time2gpst( rtcm->time, &week ); //to insert GPS time week into glonass sys
	...
	{
		/* beidou time (tow-ms) */
		epoch = ROUND_U( time2bdt( gpst2bdt( rtcm->time ), &week ) * 1E3 );
	}
	else
	{
		/* gps, qzs and galileo time (tow-ms) */
		epoch = ROUND_U( time2gpst( rtcm->time, &week ) * 1E3 );
	}
	...
	/* generate msm signal data fields */
	gen_msm_sig( rtcm, sys, *nsat, nsig, *ncell, sat_ind, sig_ind, cell_ind,
			rrng, rrate, psrng, phrng, rate, lock, half, cnr );
...
static int encode_msm_rrate( rtcm_t *rtcm, int i, const double *rrate,
		int nsat )
{
	int j, rrate_val;
...
static int encode_msm_psrng( rtcm_t *rtcm, int i, const double *psrng,
		int ncell )
{
	int j, psrng_val;
...
static int encode_msm_phrng( rtcm_t *rtcm, int i, const double *phrng,
		int ncell )
{
	int j, phrng_val;
...
{
	int j, lock_val = 0;
	//DoNothing( &lock_val ); //to get rid of warning
	for( j = 0; j < ncell; j++ )
	{
		lock_val = to_msm_lock( lock[ j ] );
		//setbitu(rtcm->buff,i,4,lock_val); i+=4;
		setbitu( rtcm->buff, i, 4, 15 );
		i += 4;
	}
...
static int encode_msm1( rtcm_t *rtcm, int sys, int sync )
{
	...
	/* encode msm header */
	if( !( i = encode_msm_head( 1, rtcm, sys, sync, &nsat, &ncell, rrng, rrate,
			NULL, psrng, NULL, NULL, NULL, NULL, NULL ) ) )
	{
		return 0;
	}
...
static int encode_msm2( rtcm_t *rtcm, int sys, int sync )
{
	...
	/* encode msm header */
	if( !( i = encode_msm_head( 2, rtcm, sys, sync, &nsat, &ncell, rrng, rrate,
			NULL, NULL, phrng, NULL, lock, half, NULL ) ) )
	{
		return 0;
	}
	/* encode msm satellite data */
...
static int encode_msm3( rtcm_t *rtcm, int sys, int sync )
{
	...
	/* encode msm header */
	if( !( i = encode_msm_head( 3, rtcm, sys, sync, &nsat, &ncell, rrng, rrate,
			NULL, psrng, phrng, NULL, lock, half, NULL ) ) )
	{
		return 0;
	}
	/* encode msm satellite data */
...
static int encode_msm4( rtcm_t *rtcm, int sys, int sync )
{
	...
	//logd("666666 start encode_msm_head sys=%d sync=%d\n", sys, sync );
	/* encode msm header */
	if( !( i = encode_msm_head( 4, rtcm, sys, sync, &nsat, &ncell, rrng, rrate,
			NULL, psrng, phrng, NULL, lock, half, cnr ) ) )
	{
		return 0;
	}
	//logd("666666 finish encode_msm_head nsat=%d ncell=%d\n", nsat, ncell );

	/* encode msm satellite data */
	//fprintf( stderr, "about to encode sys=%d\n",sys );
...
static int encode_msm5( rtcm_t *rtcm, int sys, int sync )
{
	...
	/* encode msm header */
	if( !( i = encode_msm_head( 5, rtcm, sys, sync, &nsat, &ncell, rrng, rrate,
			info, psrng, phrng, rate, lock, half, cnr ) ) )
	{
		return 0;
	}
	/* encode msm satellite data */
...
static int encode_msm6( rtcm_t *rtcm, int sys, int sync )
{
	...
	/* encode msm header */
	if( !( i = encode_msm_head( 6, rtcm, sys, sync, &nsat, &ncell, rrng, rrate,
			NULL, psrng, phrng, NULL, lock, half, cnr ) ) )
	{
		return 0;
	}
...
static int encode_msm7( rtcm_t *rtcm, int sys, int sync )
{
	...
	/* encode msm header */
	if( !( i = encode_msm_head( 7, rtcm, sys, sync, &nsat, &ncell, rrng, rrate,
			info, psrng, phrng, rate, lock, half, cnr ) ) )
	{
		return 0;
	}
